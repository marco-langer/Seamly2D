...
void noisyFailureMsgHandler(QtMsgType type, const QMessageLogContext& context, const QString& msg)
{
...

    {
        ...
        switch (type) {
        case QtDebugMsg:
            debugdate += QString(":DEBUG:%1(%2)] %3: %4: %5")
                             .arg(context.file)
                             .arg(context.line)
            ...
            break;
        case QtWarningMsg:
            debugdate += QString(":WARNING:%1(%2)] %3: %4: %5")
                             .arg(context.file)
                             .arg(context.line)
                             .arg(context.function)
                             .arg(context.category)
            ...
            break;
        case QtCriticalMsg:
            debugdate += QString(":CRITICAL:%1(%2)] %3: %4: %5")
                             .arg(context.file)
                             .arg(context.line)
                             .arg(context.function)
                             .arg(context.category)
            ...
            break;
        case QtFatalMsg:
            debugdate += QString(":FATAL:%1(%2)] %3: %4: %5")
                             .arg(context.file)
                             .arg(context.line)
                             .arg(context.function)
                             .arg(context.category)
        ...
        case QtInfoMsg:
            debugdate += QString(":INFO:%1(%2)] %3: %4: %5")
                             .arg(context.file)
                             .arg(context.line)
                             .arg(context.function)
                             .arg(context.category)
    ...
    , m_out(nullptr)
{
    ...
    // making sure will create new instance...just in case we will ever do 2 objects of
    // Application2D
    VCommandLine::Reset();
...
void Application2D::startNewSeamly2D(const QString& fileName)
{
    ...
    if (fileName.isEmpty()) {
        ...
        const QString run =
            QString("\"%1\" \"%2\"").arg(QCoreApplication::applicationFilePath()).arg(fileName);
        qCDebug(vApp, "New process with arguments. program = %s", qUtf8Printable(run));

        if (QProcess::startDetached(
                QCoreApplication::applicationFilePath(), QStringList{ fileName })) {
            qCDebug(vApp, "The process was started successfully.");
        } else {
            qCWarning(vApp, "Could not run process. The operation timed out or an error occurred.");
        }
...
bool Application2D::notify(QObject* receiver, QEvent* event)
{
    ...
    // These last two cases are special. I found that we can't show here a modal dialog with an
    // error message. Somehow program doesn't wait until an error dialog will be closed. But if
    // ignore the exception the program will hang.
...
QString Application2D::logDirPath() const
{
    ...
    const QString logDirPath =
        QStandardPaths::locate(
            QStandardPaths::GenericDataLocation, QString(), QStandardPaths::LocateDirectory)
        + "Seamly2D";
#else
    const QString logDirPath =
        QStandardPaths::locate(
            QStandardPaths::ConfigLocation, QString(), QStandardPaths::LocateDirectory)
...
void Application2D::beginLogging()
{
    ...
    if (m_lockLog->IsLocked()) {
        if (m_lockLog->GetProtected()->open(
            ...
            if (info.birthTime().daysTo(QDateTime::currentDateTime()) >= DAYS_TO_KEEP_LOGS) {
                ...
                if (tmp.GetProtected() != nullptr) {
                    if (tmp.GetProtected()->remove()) {
                        ...
                        qCWarning(
                            vApp, "Could not delete %s", qUtf8Printable(info.absoluteFilePath()));
                    }
                } else {
...
//---------------------------------------------------------------------------------------------------------------------
QTextStream* Application2D::logFile() { return m_out.get(); }

//---------------------------------------------------------------------------------------------------------------------
const VTranslateVars* Application2D::translateVariables() { return m_trVars; }
 ...
 */
bool Application2D::isAppInGUIMode() const { return isGUIMode(); }

//---------------------------------------------------------------------------------------------------------------------
const VCommandLinePtr Application2D::commandLine() const { return VCommandLine::instance; }
//---------------------------------------------------------------------------------------------------------------------
