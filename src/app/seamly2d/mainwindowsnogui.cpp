...
bool MainWindowsNoGUI::LayoutSettings(VLayoutGenerator& lGenerator)
{
    ...
    if (Application2D::isGUIMode()) {
        ...
        connect(
            &lGenerator, &VLayoutGenerator::Arranged, &progress, &DialogLayoutProgress::Arranged);
        connect(&lGenerator, &VLayoutGenerator::Error, &progress, &DialogLayoutProgress::Error);
        connect(
            &lGenerator, &VLayoutGenerator::Finished, &progress, &DialogLayoutProgress::Finished);
...
void MainWindowsNoGUI::PrintPages(QPrinter* printer)
{
    ...
    const QRectF printerPageRect(
        0,
        0,
        ToPixel(printer->pageRect(QPrinter::Millimeter).width(), Unit::Mm),
    ...
    if (isTiled) {
        ...
        if (settings->getTiledPDFOrientation() == PageOrientation::Landscape) {
            painter.rotate(-90);
            painter.translate(
                -ToPixel(printer->pageRect(QPrinter::Millimeter).height(), Unit::Mm), 0);
        }
        ...
        for (int i = 0; i < scenes.size(); ++i) {
            ...
            if (paper) {
                *poster +=
                    posterazor->Calc(paper->rect().toRect(), i, settings->getTiledPDFOrientation());
            }
        }
    ...
    // Handle the fromPage(), toPage(), supportsMultipleCopies(), and numCopies() values from
    // QPrinter.
    ...
    for (int i = 0; i < copyCount; ++i) {
        for (int j = 0; j < numPages; ++j) {
            ...
            if (paper) {
                ...
                QRectF target(
                    x * scale, y * scale, source.width() * scale, source.height() * scale);
...
//---------------------------------------------------------------------------------------------------------------------
/**
 * @brief refreshGrainLines call to recalculate piece grainlines. For example after show / hide
 * grainlines.
 */
...
QIcon MainWindowsNoGUI::ScenePreview(int i) const
{
    ...
    if (paper) {
        ...
        image = QImage(
            QSize(static_cast<qint32>(r.width()), static_cast<qint32>(r.height())),
            QImage::Format_RGB32);
        ...
        if (!image.isNull()) {
            ...
            painter.setRenderHint(QPainter::Antialiasing, true);
            painter.setPen(
                QPen(Qt::black, widthMainLine, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin));
            painter.setBrush(QBrush(Qt::NoBrush));
...
//---------------------------------------------------------------------------------------------------------------------
QList<QGraphicsScene*> MainWindowsNoGUI::CreateScenes(
    const QList<QGraphicsItem*>& papers,
    const QList<QGraphicsItem*>& shadows,
    const QList<QList<QGraphicsItem*>>& pieces)
{
 ...
 * @param fileName name layout file.
 */
void MainWindowsNoGUI::exportPDF(
    const QString& name,
    ...
    const QMarginsF& margins) const
{
    QPrinter printer;
    printer.setCreator(
        QGuiApplication::applicationDisplayName() + QLatin1String(" ")
    ...
    QSizeF size(
        FromPixel(sourceRect.width() + margins.left() + margins.right(), Unit::Mm),
    ...
    if (painter.begin(&printer) == false) {
        qCritical(
            "%s", qUtf8Printable(tr("Can't open printer %1").arg(name)));   // failed to open file
        return;
    }
    painter.setFont(QFont("Arial", 8, QFont::Normal));
    painter.setRenderHint(QPainter::Antialiasing, true);
...
void MainWindowsNoGUI::PdfTiledFile(const QString& name)
{
    ...
    // Call IsPagesFit after setting a printer settings and check if pages is not bigger than
    // printer's paper size
    if (!isTiled
        && not IsPagesFit(printer.pageLayout().fullRectPixels(printer.resolution()).size())) {
 ...
 * @param fileName name layout file.
 */
void MainWindowsNoGUI::exportEPS(
    const QString& name,
    QGraphicsRectItem* paper,
    QGraphicsScene* scene,
    bool ignoreMargins,
 ...
 * @param fileName name layout file.
 */
void MainWindowsNoGUI::exportPS(
    const QString& name,
    QGraphicsRectItem* paper,
    QGraphicsScene* scene,
    bool ignoreMargins,
 ...
 * @param params string with parameter for tool. Parameters have format: "-eps input_file out_file".
 * Use -eps when need create eps file.
 */
...
//---------------------------------------------------------------------------------------------------------------------
void MainWindowsNoGUI::AAMADxfFile(
    const QString& name,
    int version,
    bool binary,
    const QSize& size,
 ...
 *
 * Because QPaintEngine::drawTextItem doesn't pass whole string per time we mark end of each string
 * by adding special placeholder. This method append it.
 *
 * @param placeholder placeholder that will be appended to each QGraphicsSimpleTextItem item's text
 * string.
 */
        ...
        for (int j = 0; j < paperItems.size(); ++j) {
            ...
            for (int k = 0; k < pieceChildren.size(); ++k) {
                ...
                if (item->type() == QGraphicsSimpleTextItem::Type) {
                    if (QGraphicsSimpleTextItem* textItem =
...
//---------------------------------------------------------------------------------------------------------------------
/**
 * @brief MainWindowsNoGUI::RestoreTextAfterDXF restore QGraphicsSimpleTextItem items after export
 * to flat dxf.
 *
 * Because QPaintEngine::drawTextItem doesn't pass whole string per time we mark end of each string
 * by adding special placeholder. This method remove it.
 *
 * @param placeholder placeholder that will be removed from each QGraphicsSimpleTextItem item's text
 * string.
 */
void MainWindowsNoGUI::RestoreTextAfterDXF(
    const QString& placeholder, const QList<QList<QGraphicsItem*>>& pieces) const
{
    for (int i = 0; i < pieces.size(); ++i) {
        const QList<QGraphicsItem*>& paperItems = pieces.at(i);
...
void MainWindowsNoGUI::SetPrinterSettings(QPrinter* printer, const PrintType& printType)
{
    ...
    if (!isTiled) {
        QSizeF size =
            QSizeF(FromPixel(paperSize.width(), Unit::Mm), FromPixel(paperSize.height(), Unit::Mm));
        if (isAutoCrop || isUnitePages) {
            auto* paper = qgraphicsitem_cast<QGraphicsRectItem*>(papers.at(0));
            if (paper) {
                size = QSizeF(
                    FromPixel(paperSize.width(), Unit::Mm),
        ...
        VSettings* settings = qApp->Seamly2DSettings();
        QSizeF size = QSizeF(
            settings->getTiledPDFPaperWidth(Unit::Mm), settings->getTiledPDFPaperHeight(Unit::Mm)
    ...
    IsLayoutGrayscale() ? printer->setColorMode(QPrinter::GrayScale)
    ...
    for (int i = 0; i < scenes.size(); ++i) {
        ...
        if (paper) {
            ...
            case LayoutExportFormat::PDF:
                ...
                break;
            case LayoutExportFormat::PNG: exportPNG(name, scene); break;
            case LayoutExportFormat::JPG: exportJPG(name, scene); break;
            case LayoutExportFormat::BMP: exportBMP(name, scene); break;
            case LayoutExportFormat::TIF: exportTIF(name, scene); break;
            case LayoutExportFormat::PPM: exportPPM(name, scene); break;
...
int MainWindowsNoGUI::ContinueIfLayoutStale()
{
    ...
    msgBox.setText(
        tr("The layout was not updated since last pattern modification. Do you want to continue?"));
