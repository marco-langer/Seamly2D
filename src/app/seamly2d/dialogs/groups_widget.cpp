...

{
    ...
    connect(
        ui->showAllGroups_ToolButton, &QToolButton::clicked, this, &GroupsWidget::showAllGroups);
    connect(
        ui->hideAllGroups_ToolButton, &QToolButton::clicked, this, &GroupsWidget::hideAllGroups);
    connect(
        ui->lockAllGroups_ToolButton, &QToolButton::clicked, this, &GroupsWidget::lockAllGroups);
    connect(
        ui->unlockAllGroups_ToolButton,
        &QToolButton::clicked,
        this,
        &GroupsWidget::unlockAllGroups);
    connect(ui->addGroup_ToolButton, &QToolButton::clicked, this, &GroupsWidget::addGroupToList);
    connect(
        ui->deleteGroup_ToolButton,
        &QToolButton::clicked,
        this,
    ...
    connect(
        ui->groups_TableWidget,
        &QTableWidget::cellDoubleClicked,
        this,
        &GroupsWidget::cellDoubleClicked);
    connect(
        ui->groups_TableWidget,
        &QTableWidget::currentCellChanged,
        this,
        &GroupsWidget::fillGroupItemList);
    ...
    connect(
        ...
        this,
        &GroupsWidget::groupContextMenu);

    ui->groupItems_ListWidget->setContextMenuPolicy(Qt::CustomContextMenu);
    connect(
        ui->groupItems_ListWidget,
        &QListWidget::customContextMenuRequested,
        this,
        &GroupsWidget::groupItemContextMenu);
    connect(
        ui->groupItems_ListWidget,
        &QListWidget::itemDoubleClicked,
        this,
        &GroupsWidget::itemDoubleClicked);

    connect(
        ui->groups_TableWidget->horizontalHeader(),
        &QHeaderView::sectionClicked,
        this,
        &GroupsWidget::headerClicked);
    connect(ui->groups_Splitter, &QSplitter::splitterMoved, this, &GroupsWidget::splitterMoved);
}

//---------------------------------------------------------------------------------------------------------------------
GroupsWidget::~GroupsWidget() { delete ui; }
...
void GroupsWidget::addGroupToList()
{
    ...
    while (true) {
        ...
        messageBox.setText(
            tr("The action can't be completed because the group name already exists."));
        int boxResult = messageBox.exec();

        switch (boxResult) {
        case QMessageBox::Retry: break;     // Repeat Add Group Dialog
        case QMessageBox::Cancel: return;   // Exit Add Group Dialog
...
void GroupsWidget::editGroup()
{
    ...
    if (ui->groups_TableWidget->rowCount() == 0 || row == -1
        || row >= ui->groups_TableWidget->rowCount()) {
        ui->groups_TableWidget->blockSignals(false);
        return;
    }
    ...
    if (locked == false) {
        ...
        while (true) {
            ...
            switch (boxResult) {
            case QMessageBox::Retry: break;   // Repeat Edit Group Dialog
        ...
        item->setFlags(
            item->flags()
            & ~Qt::ItemIsEditable);   // set the item non-editable (view only), and non-selectable
...
void GroupsWidget::fillTable(const QMap<quint32, GroupAttributes>& groups)
{
    ...
    while (i != groups.constEnd()) {
        ...
        item->setSizeHint(QSize(20, 20));
        item->setIcon(
            data.locked ? QIcon("://icon/32x32/lock_on.png") : QIcon("://icon/32x32/lock_off.png"));
        item->setData(Qt::UserRole, i.key());
        item->setFlags(
            item->flags()
            & ~Qt::ItemIsEditable);   // set the item non-editable (view only), and non-selectable
    ...
    ui->groups_TableWidget->setHorizontalHeaderItem(
        0, new QTableWidgetItem(makeHeaderName(tr("Visible"))));
    ui->groups_TableWidget->horizontalHeaderItem(0)->setToolTip(tr("Group is visible"));
    ui->groups_TableWidget->setHorizontalHeaderItem(
        1, new QTableWidgetItem(makeHeaderName(tr("Locked"))));
    ui->groups_TableWidget->horizontalHeaderItem(1)->setToolTip(tr("Group is locked"));
    ui->groups_TableWidget->setHorizontalHeaderItem(
        2, new QTableWidgetItem(makeHeaderName(tr("Objects"))));
    ui->groups_TableWidget->horizontalHeaderItem(2)->setToolTip(tr("Group has objects"));
    ui->groups_TableWidget->setHorizontalHeaderItem(
        3, new QTableWidgetItem(makeHeaderName(tr("Color"))));
    ...
    ui->groups_TableWidget->horizontalHeaderItem(4)->setTextAlignment(
...
void GroupsWidget::setAddGroupEnabled(bool value) { ui->addGroup_ToolButton->setEnabled(value); }
...
void GroupsWidget::addGroupItem(const quint32& toolId, const quint32& objId, const Tool& tooltype)
{
    ...
    try {
        ...
        case Tool::ArcIntersectAxis:   // Same as Tool::CurveIntersectAxis, but tool will never have
                                       // such type
        ...
        case Tool::Rotation: {
            ...
            objName = tr("%1 - Rotation").arg(getObjName(objId == NULL_ID ? toolId : objId));
            break;
        }
...
QString GroupsWidget::getObjName(quint32 toolId)
{
    try {
        ...
        qCDebug(
            WidgetGroups,
            "Error! Couldn't get object name by id = %s. %s %s",
...
void GroupsWidget::groupItemContextMenu(const QPoint& pos)
{
    ...
    if (ui->groupItems_ListWidget->count() == 0 || row == -1
        || row >= ui->groupItems_ListWidget->count()) {
        return;
    }
    ...
    QMap<quint32, QString> groupsNotContainingItem =
        m_doc->getGroupsContainingItem(toolId, objId, false);
    QActionGroup* actionMoveGroupMenu = new QActionGroup(this);

    if (!groupsNotContainingItem.empty()) {
        QMenu* menuMoveGroupItem =
            menu.addMenu(QIcon("://icon/32x32/list-move_32.png"), tr("Move Group Object"));
    ...
    QAction* actionRemove =
        menu.addAction(QIcon::fromTheme("edit-delete"), tr("Remove Group Object"));
    ...
    if (selectedAction == nullptr) {
        ...
        if (locked == false) {
            ...
            SCASSERT(window != nullptr)
            {
                ...
                connect(
                    command,
                    &RemoveGroupItem::updateGroups,
                    window,
                    &VAbstractMainWindow::updateGroups);
                qApp->getUndoStack()->push(command);
            }
        }
        ...
        if ((sourceLock == false)
            && (destinationLock == false))   // only move if both groups are unlocked
        {
            qCDebug(
                WidgetGroups,
            ...
            QDomElement sourceItem =
                m_doc->removeGroupItem(itemData.last(), itemData.first(), groupId);
            QDomElement destItem =
                m_doc->addGroupItem(itemData.last(), itemData.first(), destinationGroupId);
            ...
            SCASSERT(window != nullptr)
            {
                MoveGroupItem* command =
                    new MoveGroupItem(sourceItem, destItem, m_doc, groupId, destinationGroupId);
                connect(
                    command,
...
void GroupsWidget::setGroupVisibility(
    QTableWidgetItem* item, const quint32& groupId, const bool& visible)
{
