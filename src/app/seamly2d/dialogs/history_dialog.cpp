    ...
    , m_cursorToolRecordRow(0)
{
    ...
    setWindowFlags(
        (windowFlags() | Qt::WindowStaysOnTopHint | Qt::WindowMaximizeButtonHint)
...
//---------------------------------------------------------------------------------------------------------------------
/**
 * @brief changedCursor changed cursor of input. Cursor show after which record we will insert new
 * object
...
void HistoryDialog::fillTable()
{
    ...
    if (count > 0) {
        ...
        item->setIcon(
            QIcon("://icon/24x24/left_to_right_arrow.png"));   // place arrow in 1st column of most
...
RowData HistoryDialog::record(const VToolRecord& tool)
{
    ...
    try {
        ...
        case Tool::ArcIntersectAxis:   // Same as Tool::CurveIntersectAxis, but tool will never has
                                       // such type
        ...
        case Tool::Rotation:
            ...
            rowData.tool = tr("Rotation around point %1. Suffix %2")
                               .arg(
                                   getPointName(attrUInt(domElement, AttrCenter)),
        ...
        case Tool::MirrorByLine:
            ...
            rowData.tool = tr("Mirror by Line %1_%2. Suffix %3")
                               .arg(getPointName(attrUInt(domElement, AttrP1Line)))
                               .arg(
                                   getPointName(attrUInt(domElement, AttrP2Line)),
                                   m_doc->GetParametrString(domElement, AttrSuffix, QString()));
            break;
        ...
        // Because "history" not only shows history of pattern, but helps restore current data for
        // each pattern's piece, we need add record about details and nodes, but don't show them.
        case Tool::Piece:
        case Tool::Union:
        case Tool::NodeArc:
        case Tool::NodeElArc:
