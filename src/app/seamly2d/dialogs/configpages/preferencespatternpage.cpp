...
//---------------------------------------------------------------------------------------------------------------------
PreferencesPatternPage::~PreferencesPatternPage() { delete ui; }
...
void PreferencesPatternPage::Apply()
{
    ...
    settings->setDefaultInternalLinetype(
        ui->defaultInternalLinetype_ComboBox->currentData().toString());
    settings->setDefaultInternalLineweight(
        ui->defaultInternalLineweight_ComboBox->currentData().toReal());

    settings->setDefaultCutoutColor(ui->defaultCutoutColor_ComboBox->currentData().toString());
    settings->setDefaultCutoutLinetype(
        ui->defaultCutoutLinetype_ComboBox->currentData().toString());
    settings->setDefaultCutoutLineweight(
        ui->defaultCutoutLineweight_ComboBox->currentData().toReal());
    ...
    settings->setDefaultGrainlineColor(
        ui->defaultGrainlineColor_ComboBox->currentData().toString());
    settings->setDefaultGrainlineLineweight(
        ui->defaultGrainlineLineweight_ComboBox->currentData().toReal());
...
void PreferencesPatternPage::initDefaultSeamAllowance()
{
    ...
    ui->showSeamAllowances_CheckBox->setChecked(
        qApp->Seamly2DSettings()->getDefaultSeamAllowanceVisibilty());
    ui->defaultSeamAllowance_DoubleSpinBox->setValue(
        qApp->Seamly2DSettings()->GetDefaultSeamAllowance());
    ui->defaultSeamAllowance_DoubleSpinBox->setSuffix(
        " " + UnitsToStr(StrToUnits(qApp->Seamly2DSettings()->GetUnit()), true));

    int index =
        ui->defaultSeamColor_ComboBox->findData(qApp->Seamly2DSettings()->getDefaultSeamColor());
    if (index != -1) {
        ui->defaultSeamColor_ComboBox->setCurrentIndex(index);
    }
    index = ui->defaultSeamLinetype_ComboBox->findData(
        qApp->Seamly2DSettings()->getDefaultSeamLinetype());
    if (index != -1) {
        ui->defaultSeamLinetype_ComboBox->setCurrentIndex(index);
    }
    index = ui->defaultSeamLineweight_ComboBox->findData(
...
void PreferencesPatternPage::editDateTimeFormats()
{
    ...
    if (button == ui->editDateFormats_PushButton) {
        callDateTimeFormatEditor(
            QDate::currentDate(),
            settings->PredefinedDateFormats(),
            settings->GetUserDefinedDateFormats(),
            ui->dateFormats_ComboBox);
    } else if (button == ui->editTimeFormats_PushButton) {
        callDateTimeFormatEditor(
            QTime::currentTime(),
            settings->PredefinedTimeFormats(),
            settings->GetUserDefinedTimeFormats(),
            ui->timeFormats_ComboBox);
    }
}
...
void PreferencesPatternPage::initializeLabelsTab()
{
    ...
    ui->defaultLabelWidth_DoubleSpinBox->setSuffix(
        " " + UnitsToStr(StrToUnits(qApp->Seamly2DSettings()->GetUnit()), true));
    ui->defaultLabelHeight_DoubleSpinBox->setValue(
        qApp->Seamly2DSettings()->getDefaultLabelHeight());
    ui->defaultLabelHeight_DoubleSpinBox->setSuffix(
        " " + UnitsToStr(StrToUnits(qApp->Seamly2DSettings()->GetUnit()), true));

    int index =
        ui->defaultLabelColor_ComboBox->findData(qApp->Seamly2DSettings()->getDefaultLabelColor());
    if (index != -1) {
    ...
    connect(
        ...
        &QPushButton::clicked,
        this,
        &PreferencesPatternPage::editDateTimeFormats);
    connect(
    ...
    connect(
        ...
        &QToolButton::clicked,
        this,
        &PreferencesPatternPage::setDefaultTemplate);
    connect(
...
void PreferencesPatternPage::initNotches()
{
    ...
    //-----------------------  Get Default Notch
    int index =
        ui->defaultNotchType_ComboBox->findData(qApp->Seamly2DSettings()->getDefaultNotchType());
    if (index != -1) {
        ui->defaultNotchType_ComboBox->setCurrentIndex(index);
    }

    index =
        ui->defaultNotchColor_ComboBox->findData(qApp->Seamly2DSettings()->getDefaultNotchColor());
    if (index != -1) {
    ...
    ui->showSeamAllowanceNotch_CheckBox->setChecked(
        qApp->Seamly2DSettings()->showSeamAllowanceNotch());
    ui->defaultNotchLength_DoubleSpinBox->setValue(
        qApp->Seamly2DSettings()->getDefaultNotchLength());
    ui->defaultNotchLength_DoubleSpinBox->setSuffix(
        " " + UnitsToStr(StrToUnits(qApp->Seamly2DSettings()->GetUnit()), true));
    ui->defaultNotchWidth_DoubleSpinBox->setValue(qApp->Seamly2DSettings()->getDefaultNotchWidth());
    ui->defaultNotchWidth_DoubleSpinBox->setSuffix(
        " " + UnitsToStr(StrToUnits(qApp->Seamly2DSettings()->GetUnit()), true));
}
...
{
    ui->showGrainlines_CheckBox->setChecked(
        qApp->Seamly2DSettings()->getDefaultGrainlineVisibilty());

    ui->defaultGrainlineLength_DoubleSpinBox->setValue(
        qApp->Seamly2DSettings()->getDefaultGrainlineLength());
    ui->defaultGrainlineLength_DoubleSpinBox->setSuffix(
        " " + UnitsToStr(StrToUnits(qApp->Seamly2DSettings()->GetUnit()), true));

    int index = ui->defaultGrainlineColor_ComboBox->findData(
        qApp->Seamly2DSettings()->getDefaultGrainlineColor());
    if (index != -1) {
        ui->defaultGrainlineColor_ComboBox->setCurrentIndex(index);
    }
    index = ui->defaultGrainlineLineweight_ComboBox->findData(
...
//---------------------------------------------------------------------------------------------------------------------
void PreferencesPatternPage::initComboBoxFormats(
    QComboBox* box, const QStringList& items, const QString& currentFormat)
{
...
//---------------------------------------------------------------------------------------------------------------------
template <typename T>
void PreferencesPatternPage::callDateTimeFormatEditor(
    const T& type,
