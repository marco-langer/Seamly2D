    ...
    , m_allPieces()
{
    ...
    connect(
        ui->includeAllPieces_ToolButton,
        &QToolButton::clicked,
        this,
    ...
    connect(
        ui->excludeAllPieces_ToolButton,
        &QToolButton::clicked,
        this,
        &PiecesWidget::excludeAllPieces);
    ...
    connect(
        ui->unlockAllPieces_ToolButton,
        &QToolButton::clicked,
        this,
        &PiecesWidget::unlockAllPieces);
    ...
    connect(
        ui->tableWidget, &QTableWidget::cellDoubleClicked, this, &PiecesWidget::cellDoubleClicked);
    connect(
        ui->tableWidget,
    ...
    connect(
        ...
        this,
        &PiecesWidget::headerClicked);
}

//---------------------------------------------------------------------------------------------------------------------
PiecesWidget::~PiecesWidget() { delete ui; }
...
//---------------------------------------------------------------------------------------------------------------------
void PiecesWidget::updateList() { fillTable(m_data->DataPieces()); }
...
void PiecesWidget::fillTable(const QHash<quint32, VPiece>* pieces)
{
    ...
    while (i != pieces->constEnd()) {
        ...
        item->setIcon(
            piece.isInLayout() ? QIcon("://icon/32x32/visible_on.png")
        ...
        item->setSizeHint(QSize(20, 20));
        item->setIcon(
            piece.isLocked() ? QIcon("://icon/32x32/lock_on.png")
        ...
        {   // Add letter item
            ...
            item->setFlags(
                item->flags() &= ~(Qt::ItemIsEditable));   // set the item non-editable (view only),
                                                           // and non-selectable
    ...
    ui->tableWidget->setHorizontalHeaderItem(
        0, new QTableWidgetItem(makeHeaderName(tr("Included"))));
        ...
        if (allPieces->contains(id)) {
            if (!(lock == allPieces->value(id).isLocked())) {
                ...
                VMainGraphicsScene* scene =
                    qobject_cast<VMainGraphicsScene*>(qApp->getCurrentScene());
                SCASSERT(scene != nullptr)
