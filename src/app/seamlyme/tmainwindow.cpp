...
//---------------------------------------------------------------------------------------------------------------------
QString TMainWindow::CurrentFile() const { return curFile; }
    ...
    if (individualMeasurements == nullptr) {
        ...
        try {
            ...
            if (mType == MeasurementsType::Multisize) {
                ...
                individualMeasurements->setXMLContent(
                    converter.Convert());   // Read again after conversion
    ...
    if (individualMeasurements == nullptr) {
        ...
        if (mType == MeasurementsType::Multisize) {
            individualMeasurements =
                new MeasurementDoc(mUnit, measurements.baseSize(), measurements.baseHeight(), data);
            individualMeasurements->setSize(&currentSize);
            individualMeasurements->setHeight(&currentHeight);
...
void TMainWindow::OpenIndividual()
{
    const QString filter = tr("Individual measurements") + QLatin1String(" (*.") + smisExt
                         + QLatin1String(" *.") + vitExt + QLatin1String(");;") + tr("All files")
                         + QLatin1String(" (*.*)");
...
void TMainWindow::OpenMultisize()
{
    const QString filter = tr("Multisize measurements") + QLatin1String(" (*.") + smmsExt
                         + QLatin1String(" *.") + vstExt + QLatin1String(");;") + tr("All files")
                         + QLatin1String(" (*.*)");
...
{
    const QString filter = tr("Measurements") + QLatin1String(" (*.") + smisExt
                         + QLatin1String(" *.") + smmsExt + QLatin1String(" *.") + vitExt
                         + QLatin1String(" *.") + vstExt + QLatin1String(");;") + tr("All files")
                         + QLatin1String(" (*.*)");
    ...
    if (individualMeasurements != nullptr) {   // The file was opened.
...
QFileDialog::DontUseNativeDialog, QFileDialog::ExistingFile, QFileDialog::AcceptOpen);
...
{
    QString msg =
        tr("To utilize a 3DLook body scan the file needs to be converted to SeamlyME format.\n")
        + tr("Attach your 3DLook file to an email and send to convert@seamly.io.\n\n")
        + tr(
            "You will recieve an email with the converted file, which you can then\nload in "
            "SeamlyME as usual.\n\n");
    ...
    /*
        VSettings *settings = new VSettings(QSettings::IniFormat, QSettings::UserScope,
       "Seamly2DTeam", "Seamly2D", this);
        ...
        QString body = "Dear Seamly Team,\n\n Please convert the following 3DLook files to SeamlyME
       format.\n";
...
void TMainWindow::changeEvent(QEvent* event)
{
    if (event->type() == QEvent::LanguageChange) {
        ...
        if (mType == MeasurementsType::Multisize) {
            ...
            ui->labelBaseSizeValue->setText(
                QString().setNum(individualMeasurements->BaseSize()) + QLatin1String(" ")
                + UnitsToStr(individualMeasurements->measurementUnits(), true));
            ui->labelBaseHeightValue->setText(
                QString().setNum(individualMeasurements->BaseHeight()) + QLatin1String(" ")
                + UnitsToStr(individualMeasurements->measurementUnits(), true));
...
void TMainWindow::printPages(QPrinter* printer)
{
    ...
    if (mType == MeasurementsType::Multisize) {
        text.append(
            "<tr><td align = left><b>Base Size:</b></td><td>" + ui->labelBaseSizeValue->text()
            + "</td></tr>");
        text.append(
            "<tr><td align = left><b>Base Height:</b></td><td>" + ui->labelBaseHeightValue->text()
            + "</td></tr>");
    } else {
        text.append(
            "<tr><td align = left><b>Units:</b></td><td>" + UnitsToStr(mUnit) + "</td></tr>");
        text.append(
            "<tr><td align = left><b>First Name:</b></td><td>" + ui->lineEditGivenName->text()
            + "</td></tr>");
        text.append(
            "<tr><td align = left><b>Last Name:</b></td><td>" + ui->lineEditFamilyName->text()
            + "</td></tr>");
        text.append(
            "<tr><td align = left><b>Gender:</b></td><td>" + ui->comboBoxGender->currentText()
            + "</td></tr>");
        text.append(
            "<tr><td align = left><b>Email:</b></td><td>" + ui->lineEditEmail->text()
            + "</td></tr>");
    }
    text.append(
        "<tr><td align = left><b>Notes:</b></td><td>" + ui->plainTextEditNotes->toPlainText()
        + "</td></tr></table>");
    text.append("<p>");
    ...
    for (int i = 0; i < columns; i++) {
        text.append("<th>")
            .append(ui->tableWidget->horizontalHeaderItem(i)->data(Qt::DisplayRole).toString())
            .append("</th>");
    }
        ...
        for (int j = 0; j < columns; j++) {
            ...
            if (j == 1 || j > 2) {
                text.append("<td align = center>")
                    .append(ui->tableWidget->item(i, j)->text())
                    .append("</td>");
            } else {
        ...
        if (!isFileWritable) {
            ...
            if (messageBox.exec() == QMessageBox::Yes) {
                ...
                bool changed = QFile::setPermissions(
                    curFile, QFileInfo(curFile).permissions() | QFileDevice::WriteUser);
#ifdef Q_OS_WIN32
                qt_ntfs_permission_lookup--;   // turn it off again
#endif                                         /*Q_OS_WIN32*/
                ...
                if (!changed) {
                    ...
                    messageBox.setText(
                        tr("Cannot set permissions for %1 to writable.").arg(curFile));
...
void TMainWindow::Fx()
{
    ...
    if (dialog->exec() == QDialog::Accepted) {
        ...
        // pointer.
        const QTableWidgetItem* nameField = ui->tableWidget->item(row, ColumnName);
        individualMeasurements->SetMValue(
            nameField->data(Qt::UserRole).toString(), dialog->GetFormula());
...
void TMainWindow::AddKnown()
{
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        if (ui->tableWidget->currentRow() == -1) {
            ...
            for (int i = 0; i < list.size(); ++i) {
                if (mType == MeasurementsType::Individual) {
                    individualMeasurements->addEmpty(
                        list.at(i), qApp->translateVariables()->MFormula(list.at(i)));
                } else {
            ...
            for (int i = 0; i < list.size(); ++i) {
                if (mType == MeasurementsType::Individual) {
                    individualMeasurements->AddEmptyAfter(
                        after, list.at(i), qApp->translateVariables()->MFormula(list.at(i)));
                } else {
...
void TMainWindow::ImportFromPattern()
{
    ...
    if (!lock.IsLocked()) {
        qCCritical(
            tMainWindow, "%s", qUtf8Printable(tr("This file already opened in another window.")));
        return;
    }
...
//---------------------------------------------------------------------------------------------------------------------
void TMainWindow::ShowMData() { ShowNewMData(true); }
        ...
        if (mType == MeasurementsType::Multisize) {
            ...
            try {
                formula = qApp->translateVariables()->FormulaToUser(
                    meash->GetFormula(), qApp->Settings()->getOsSeparator());
...
void TMainWindow::ShowMDiagram(const QString& name)
{
    ...
    if (number.isEmpty()) {
        ui->labelDiagram->setText(
            tr("<html><head/><body><p><span style=\" font-size:340pt;\">?</span></p>"
               "<p align=\"center\">Unknown measurement</p></body></html>"));
    } else {
        ui->labelDiagram->setText(
            QString("<html><head/><body><p align=\"center\">%1</p>"
...
void TMainWindow::SaveMName(const QString& text)
{
    ...
    if (meash->isCustom()) {
        ...
        qCWarning(
            tMainWindow,
            "%s",
            qUtf8Printable(tr("The name of known measurement forbidden to change.")));
    }
}
...
void TMainWindow::SaveMDescription()
{
    ...
    const QTableWidgetItem* nameField =
        ui->tableWidget->item(ui->tableWidget->currentRow(), ColumnName);
    individualMeasurements->SetMDescription(
        nameField->data(Qt::UserRole).toString(), ui->plainTextEditDescription->toPlainText());
...
void TMainWindow::SaveMFullName()
{
    ...
    if (meash->isCustom()) {
        individualMeasurements->SetMFullName(
            nameField->data(Qt::UserRole).toString(), ui->lineEditFullName->text());
        ...
        qCWarning(
            tMainWindow,
            "%s",
            qUtf8Printable(tr("The full name of known measurement forbidden to change.")));
    }
}
...
void TMainWindow::SetupMenu()
{
    ...
    connect(
        ui->actionCreateFromExisting, &QAction::triggered, this, &TMainWindow::CreateFromExisting);

    // connect(ui->bodyScanner1_Action, &QAction::triggered, this,
    // &TMainWindow::handleBodyScanner1);
    ...
    connect(
        ui->actionImportFromPattern, &QAction::triggered, this, &TMainWindow::ImportFromPattern);
...
void TMainWindow::InitWindow()
{
    ...
    if (mType == MeasurementsType::Multisize) {
        ui->labelMType->setText(tr("Multisize measurements"));
        ui->labelBaseSizeValue->setText(
            QString().setNum(individualMeasurements->BaseSize()) + " "
            + UnitsToStr(individualMeasurements->measurementUnits(), true));
        ui->labelBaseHeightValue->setText(
            QString().setNum(individualMeasurements->BaseHeight()) + " "
            + UnitsToStr(individualMeasurements->measurementUnits(), true));

        // Because Qt Designer doesn't know about our deleting we will create empty objects for
        // correct working the retranslation UI Tab Measurements
        ...
        InitGender(ui->comboBoxGender);
        const qint32 index =
            ui->comboBoxGender->findData(static_cast<int>(individualMeasurements->Gender()));
        ui->comboBoxGender->setCurrentIndex(index);
        ...
        connect(
            ui->lineEditGivenName, &QLineEdit::editingFinished, this, &TMainWindow::SaveGivenName);
        connect(
            ui->lineEditFamilyName,
            &QLineEdit::editingFinished,
            this,
            &TMainWindow::SaveFamilyName);
    ...
    InitPMSystems(ui->comboBoxPMSystem);
    const qint32 index =
        ui->comboBoxPMSystem->findData(QLatin1Char('p') + individualMeasurements->PMSystem());
    ui->comboBoxPMSystem->setCurrentIndex(index);
    connect(
        ui->comboBoxPMSystem,
        static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged),
    ...
    connect(ui->lineEditFind, &QLineEdit::textChanged, [this](const QString& term) {
        search->Find(term);
    });
    connect(
        ui->toolButtonFindPrevious, &QToolButton::clicked, [this]() { search->FindPrevious(); });
    ...
    connect(ui->lineEditName, &QLineEdit::textEdited, this, &TMainWindow::SaveMName);
    connect(
        ui->plainTextEditDescription,
        &QPlainTextEdit::textChanged,
        this,
        &TMainWindow::SaveMDescription);
...
//---------------------------------------------------------------------------------------------------------------------
QTableWidgetItem*
TMainWindow::AddCell(const QString& text, int row, int column, int aligment, bool ok)
{
...
void TMainWindow::RefreshTable(bool freshCall)
{
    ...
    for (iMap = orderedTable.constBegin(); iMap != orderedTable.constEnd(); ++iMap) {
        ...
        if (mType == MeasurementsType::Individual) {
            QTableWidgetItem* item = AddCell(
                qApp->translateVariables()->MToUser(meash->GetName()),
                currentRow,
                ColumnName,
            ...
            if (meash->isCustom()) {
                ...
                AddCell(
                    getMeasurementNumber(meash->GetName()),
                    currentRow,
                    ColumnNumber,
                    Qt::AlignVCenter);
                AddCell(
                    qApp->translateVariables()->guiText(meash->GetName()),
                    currentRow,
                    ColumnFullName,
                    Qt::AlignVCenter);
            }
            ...
            AddCell(
                locale().toString(meash->GetBase()),
                currentRow,
                ColumnBaseValue,
                Qt::AlignHCenter | Qt::AlignVCenter);   // base value

            AddCell(
                locale().toString(meash->GetKsize()),
                currentRow,
                ColumnInSizes,
                Qt::AlignHCenter | Qt::AlignVCenter);   // in sizes

            AddCell(
                locale().toString(meash->GetKheight()),
                currentRow,
                ColumnInHeights,
...
QString TMainWindow::GetCustomName() const
{
    ...
    QString name;
    do {
        name = CustomMSign + qApp->translateVariables()->InternalVarToUser(measurement_)
             + QString().number(num);
        num++;
...
void TMainWindow::UpdateWindowTitle()
{
    ...
    static QIcon fileIcon = QIcon(
        QCoreApplication::applicationDirPath() + QLatin1String("/../Resources/measurements.icns"));
    QIcon icon;
    if (!curFile.isEmpty()) {
        if (!isWindowModified()) {
            icon = fileIcon;
...
{
    if (const QTableWidgetItem* nameField =
            ui->tableWidget->item(ui->tableWidget->currentRow(), ColumnName)) {
