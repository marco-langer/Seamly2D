...
VSpline VSpline::Move(qreal length, qreal angle, const QString& prefix) const
{
    ...
    const QPointF p2{ geo::moved(static_cast<QPointF>(GetP2()), length, angle) };
    const QPointF p3{ geo::moved(static_cast<QPointF>(GetP3()), length, angle) };
...
QVector<QPointF> VSpline::SplinePoints(
    const QPointF& p1,
    const QPointF& p4,
    qreal angle1,
    qreal angle2,
 ...
 * @return angle.
 */
qreal VSpline::GetStartAngle() const { return d->angle1; }
...
//---------------------------------------------------------------------------------------------------------------------
qreal VSpline::GetC1Length() const { return d->c1Length; }
 ...
 * @return coefficient
 */
qreal VSpline::GetKcurve() const { return d->kCurve; }
 ...
 * 1 real + 2 complex -> x[0] is real, x[1] is real part of complex roots, x[2] - non-negative
 * imaginary part.
 *
 ...
 * @return t coeffient that reprezent this point on curve. Return -1 if point doesn't belongs to
 * curve.
 */
...
//---------------------------------------------------------------------------------------------------------------------
QPointF VSpline::GetControlPoint1() const { return static_cast<QPointF>(GetP2()); }

//---------------------------------------------------------------------------------------------------------------------
QPointF VSpline::GetControlPoint2() const { return static_cast<QPointF>(GetP3()); }
