...
const QStringList builInFunctions =
    QStringList() << degTorad_F << radTodeg_F << sin_F << cos_F << tan_F << asin_F << acos_F
                  << atan_F << sinh_F << cosh_F << tanh_F << asinh_F << acosh_F << atanh_F << sinD_F
                  << cosD_F << tanD_F << asinD_F << acosD_F << atanD_F << log2_F << log10_F << log_F
                  << ln_F << exp_F << sqrt_F << sign_F << rint_F << abs_F << min_F << max_F << sum_F
                  << avg_F << fmod_F;
...
const QStringList builInPostfixOperators = QStringList() << cm_Oprt << mm_Oprt << in_Oprt;
...
const QStringList labelTemplatePlaceholders =
    QStringList() << pl_size << pl_height << pl_date << pl_time << pl_patternName
                  << pl_patternNumber << pl_author << pl_customer << pl_pExt << pl_pFileName
                  << pl_mFileName << pl_mExt << pl_pLetter << pl_pAnnotation << pl_pOrientation
                  << pl_pRotation << pl_pTilt << pl_pFoldPosition << pl_pName << pl_pQuantity
                  << pl_mFabric << pl_mLining << pl_mInterfacing << pl_mInterlining << pl_wCut
                  << pl_wOnFold;
...
// From documantation: If you use QStringLiteral you should avoid declaring the same literal in
// multiple places: This furthermore blows up the binary sizes.
...
{
    switch (unit) {
    case Unit::Mm: return (val / 25.4) * PrintDPI;
    case Unit::Cm: return ((val * 10.0) / 25.4) * PrintDPI;
    case Unit::Inch: return val * PrintDPI;
    case Unit::Px: return val;
    default: break;
    }
    return 0;
}
...
{
    switch (unit) {
    case Unit::Mm: return (pix / PrintDPI) * 25.4;
    case Unit::Cm: return ((pix / PrintDPI) * 25.4) / 10.0;
    case Unit::Inch: return pix / PrintDPI;
    case Unit::Px: return pix;
    default: break;
    }
    return 0;
}
...
qreal UnitConvertor(qreal value, const Unit& from, const Unit& to)
{
    ...
    case Unit::Mm:
        ...
        case Unit::Inch: return value / 25.4;
        case Unit::Px: return (value / 25.4) * PrintDPI;
        default: break;
        }
        break;
    case Unit::Cm:
        switch (to) {
        case Unit::Mm: return value * 10.0;
        case Unit::Cm: return value;
        case Unit::Inch: return value / 2.54;
        case Unit::Px: return ((value * 10.0) / 25.4) * PrintDPI;
        default: break;
        }
        break;
    case Unit::Inch:
        switch (to) {
        case Unit::Mm: return value * 25.4;
        case Unit::Cm: return value * 2.54;
        case Unit::Inch: return value;
        case Unit::Px: return value * PrintDPI;
        default: break;
        }
        break;
    case Unit::Px:
        switch (to) {
        case Unit::Mm: return (value / PrintDPI) * 25.4;
        case Unit::Cm: return ((value / PrintDPI) * 25.4) / 10.0;
        case Unit::Inch: return value / PrintDPI;
...
{
    const QStringList locales = QStringList() << QStringLiteral("ru_RU") << QStringLiteral("uk_UA")
                                              << QStringLiteral("de_DE") << QStringLiteral("cs_CZ")
                                              << QStringLiteral("he_IL") << QStringLiteral("fr_FR")
                                              << QStringLiteral("it_IT") << QStringLiteral("nl_NL")
                                              << QStringLiteral("id_ID") << QStringLiteral("es_ES")
                                              << QStringLiteral("fi_FI") << QStringLiteral("en_US")
                                              << QStringLiteral("en_CA") << QStringLiteral("en_IN")
                                              << QStringLiteral("ro_RO") << QStringLiteral("zh_CN")
                                              << QStringLiteral("pt_BR") << QStringLiteral("el_GR")
...
void ShowInGraphicalShell(const QString& filePath)
{
    ...
    QProcess::startDetached(
        QStringLiteral("explorer"), QStringList{ "/select", QDir::toNativeSeparators(filePath) });
#else
...
{
    switch (type) {
    case NotchType::Slit: return strSlit;
    case NotchType::TNotch: return strTNotch;
    case NotchType::UNotch: return strUNotch;
    case NotchType::VInternal: return strVInternal;
...
{
    const QStringList values = QStringList() << strSlit << strTNotch << strUNotch << strVInternal
                                             << strVExternal << strCastle << strDiamond;

    switch (values.indexOf(value)) {
    case 0: return NotchType::Slit;
    case 1: return NotchType::TNotch;
    case 2: return NotchType::UNotch;
    case 3: return NotchType::VInternal;
    case 4: return NotchType::VExternal;
    case 5: return NotchType::Castle;
    case 6: return NotchType::Diamond;
    default: break;
    }
...
NotchSubType stringToNotchSubType(const QString& value)
{
    const QStringList values = QStringList()
                            << strStraightforward << strBisector << strIntersection;

    switch (values.indexOf(value)) {
    case 0: return NotchSubType::Straightforward;
    case 1: return NotchSubType::Bisector;
    case 2: return NotchSubType::Intersection;
    default: break;
    }
...
QString UnitsToStr(const Unit& unit, const bool translate)
{
    QString result;
    switch (unit) {
    case Unit::Mm: translate ? result = QObject::tr("mm") : result = unitMM; break;
    case Unit::Inch: translate ? result = QObject::tr("inch") : result = unitINCH; break;
    case Unit::Px: translate ? result = QObject::tr("px") : result = unitPX; break;
    case Unit::Cm:
    default: translate ? result = QObject::tr("cm") : result = unitCM; break;
    }
    return result;
}
...
void InitLanguages(QComboBox* combobox)
{
    ...
    QDirIterator it(
        qApp->translationsPath(),
        QStringList("seamly2d_*.qm"),
        QDir::Files,
