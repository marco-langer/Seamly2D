...
VPE::VPropertyModel::VPropertyModel(QObject* parent)
    : QAbstractItemModel(parent)
    , d_ptr(new VPropertyModelPrivate())
{}
...
bool VPE::VPropertyModel::addProperty(
    VProperty* property, const QString& id, const QString& parentid, bool emitsignals)
{
    if (!property) {
        return false;
    }
    ...
    if (emitsignals) {
        ...
        int tmpRow = tmpParent != nullptr ? tmpParent->getRowCount()
                                          : d_ptr->Properties->getRootPropertyCount();
        beginInsertRows(
            (tmpParent != nullptr ? getIndexFromProperty(tmpParent) : QModelIndex()),
            tmpRow,
            tmpRow);
    }
...
VPE::VProperty* VPE::VPropertyModel::createProperty(
    const QString& id, const QString& name, const QString& parentid, const QVariant& data)
{
...
QModelIndex VPE::VPropertyModel::parent(const QModelIndex& index) const
{
    ...
    if (childItem) {
        ...
        if (parentItem) {
            ...
            int parents_row = grandParentItem != nullptr
...
bool VPE::VPropertyModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (index.column() == 1 && tmpProperty) {
        ...
        if (tmpProperty->getUpdateParent()
            && tmpHasChanged) {   // If neccessary, update the parent as well
...
const VPE::VPropertySet* VPE::VPropertyModel::getPropertySet() const { return d_ptr->Properties; }

void VPE::VPropertyModel::clear(bool emit_signals) { setPropertySet(nullptr, emit_signals); }

VPE::VPropertySet*
VPE::VPropertyModel::takePropertySet(VPropertySet* new_property_set, bool emit_signals)
{
