...
public:
    ...
    //! If you reimplement this in a sub property, make sure to return true or the delegate will
    //! draw the item.
    virtual bool
    paint(QPainter*, const QStyleOptionViewItem&, const QModelIndex&, const QAbstractItemDelegate*)
        const;
    ...
    virtual QWidget*
    createEditor(QWidget* parent, const QStyleOptionViewItem&, const QAbstractItemDelegate*);
    ...
    /*//! Returns a pointer to the data stored and handled by this property. In most cases this
    function shouldn't be
    //! used.
    //! \return Returns a void pointer to the data. Not all properties have to support this. By
    default, this
    ...
    //! Sets the settings by calling the overloaded setSetting(const QString &key, const QVariant
    //! &value) for each item in the map.
    virtual void setSettings(const QMap<QString, QVariant>& settings);

    //! Get the settings.
    virtual QMap<QString, QVariant> getSettings() const;

    //! Sets the settings. This function has to be implemented in a subclass in order to have an
    //! effect
    ...
    clone(bool include_children = true, VProperty* container = nullptr) const;
