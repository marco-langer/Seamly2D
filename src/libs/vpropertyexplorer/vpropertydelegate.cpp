...
QWidget* VPE::VPropertyDelegate::createEditor(
...
void VPE::VPropertyDelegate::setModelData(
...
QSize VPE::VPropertyDelegate::sizeHint(
    const QStyleOptionViewItem& option, const QModelIndex& index) const
{
    ...
    if (RowHeight > 0) {
        return QSize(
            tmpStandardSizeHint.width(),
            AddRowHeight ? tmpStandardSizeHint.height() + RowHeight : RowHeight);
    } else
...
void VPE::VPropertyDelegate::paint(
    QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
    bool done = false;
    if (index.isValid() && index.column() == 1) {
        done = reinterpret_cast<VProperty*>(index.internalPointer())
                   ->paint(painter, option, index, this);
    }
    ...
    QColor tmpPenColor = static_cast<QRgb>(
    ...
    painter->drawLine(
        option.rect.right(), option.rect.y(), option.rect.right(), option.rect.bottom());
    painter->drawLine(
        option.rect.x(), option.rect.bottom(), option.rect.right(), option.rect.bottom());
