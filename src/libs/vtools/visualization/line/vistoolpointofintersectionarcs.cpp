...
{
    if (arc1Id > NULL_ID) {
        const auto& arc1{ *Visualization::data->GeometricObject<VArc>(arc1Id) };
        DrawPath(
            arc1Path,
            arc1.GetPath(),
            arc1.DirectionArrows(),
            Qt::darkGreen,
            Qt::SolidLine,
            lineWeight,
            Qt::RoundCap);
        ...
        if (arc2Id > NULL_ID) {
            const auto& arc2{ *Visualization::data->GeometricObject<VArc>(arc2Id) };
            DrawPath(
                arc2Path,
                arc2.GetPath(),
                arc2.DirectionArrows(),
                Qt::darkRed,
                Qt::SolidLine,
                lineWeight,
                Qt::RoundCap);

            const QPointF fPoint{ VToolPointOfIntersectionArcs::FindPoint(
                &arc1, &arc2, crossPoint) };
...
//---------------------------------------------------------------------------------------------------------------------
void VisToolPointOfIntersectionArcs::setArc1Id(const quint32& value) { arc1Id = value; }

//---------------------------------------------------------------------------------------------------------------------
void VisToolPointOfIntersectionArcs::setArc2Id(const quint32& value) { arc2Id = value; }
