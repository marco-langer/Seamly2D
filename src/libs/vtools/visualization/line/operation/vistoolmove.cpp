...
void VisToolMove::RefreshGeometry()
{
    ...
    if (qFuzzyIsNull(length)) {
        if (QGuiApplication::keyboardModifiers() == Qt::ShiftModifier) {
            ...
            line.setAngle(
                CorrectAngle(line.angle()));   // contrain move line angle to preference setting
        } else {
            line = QLineF(m_origin, Visualization::scenePos);
        }
        ...
        if (VFuzzyComparePossibleNulls(rotationAngle, std::numeric_limits<int>::min())) {
            ...
            if (QGuiApplication::keyboardModifiers() == Qt::ShiftModifier) {
                rotationLine.setAngle(CorrectAngle(
                    rotationLine.angle()));   // contrain rotation line angle to preference
            }
        ...
        Visualization::toolTip = tr("Length = %1%2, angle = %3°, <b>Shift</b> to constrain angle, "
                                    "<b>Mouse click</b> - finish selecting a position")
                                     .arg(qApp->translateVariables()->FormulaToUser(
                                         QString::number(qApp->fromPixel(tempLength)),
                                         qApp->Settings()->getOsSeparator()))
                                     .arg(prefix)
                                     .arg(tempAngle);
    } else {
        Visualization::toolTip =
            tr("Length = %1%2, angle = %3°, rotation angle = %4° Hold <b>SHIFT</b> to constrain "
               "angle,"
...
//---------------------------------------------------------------------------------------------------------------------
QString VisToolMove::Angle() const { return QString::number(moveLineItem->line().angle()); }
...
//---------------------------------------------------------------------------------------------------------------------
QString VisToolMove::Rotation() const { return QString::number(rotationLineItem->line().angle()); }
...
//---------------------------------------------------------------------------------------------------------------------
QString VisToolMove::Length() const { return QString::number(LengthValue()); }
...
//---------------------------------------------------------------------------------------------------------------------
void VisToolMove::setOriginPointId(quint32 value) { object1Id = value; }
...
//---------------------------------------------------------------------------------------------------------------------
template <class Item>
int VisToolMove::AddDestinationCurve(
    qreal angle,
    qreal length,
    quint32 id,
    int i,
    ...
    for (int i = 0; i < objects.size(); ++i) {
        ...
        case GOType::Arc:
            ...
            sourceObjects.append(
                Visualization::data->GeometricObject<VAbstractCurve>(id)->getPoints());
            break;
    ...
    const QPointF& rotationOrigin)
{
    for (int i = 0; i < objects.size(); ++i) {
        ...
        case GOType::Arc:
            iCurve =
                AddDestinationCurve<VArc>(angle, length, id, iCurve, rotationAngle, rotationOrigin);
            break;
        case GOType::EllipticalArc:
            iCurve = AddDestinationCurve<VEllipticalArc>(
                angle, length, id, iCurve, rotationAngle, rotationOrigin);
            break;
        case GOType::Spline:
            iCurve = AddDestinationCurve<VSpline>(
                angle, length, id, iCurve, rotationAngle, rotationOrigin);
            break;
        case GOType::SplinePath:
            iCurve = AddDestinationCurve<VSplinePath>(
                angle, length, id, iCurve, rotationAngle, rotationOrigin);
            break;
        case GOType::CubicBezier:
            iCurve = AddDestinationCurve<VCubicBezier>(
                angle, length, id, iCurve, rotationAngle, rotationOrigin);
            break;
        case GOType::CubicBezierPath:
            iCurve = AddDestinationCurve<VCubicBezierPath>(
                angle, length, id, iCurve, rotationAngle, rotationOrigin);
            break;
        case GOType::Unknown:
        case GOType::Curve:
