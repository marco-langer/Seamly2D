    ...
    , formulaBaseHeightCircleRadius(0)
{
    ...
    connect(
        timerCircleRadius, &QTimer::timeout, this, &IntersectCircleTangentDialog::EvalCircleRadius);
    ...
    connect(
        ...
        this,
        &IntersectCircleTangentDialog::NamePointChanged);

    connect(
        ui->comboBoxCircleCenter,
        &QComboBox::currentTextChanged,
        this,
        &IntersectCircleTangentDialog::PointChanged);

    connect(
        ui->toolButtonExprRadius,
        &QPushButton::clicked,
        this,
        &IntersectCircleTangentDialog::FXCircleRadius);
...
//---------------------------------------------------------------------------------------------------------------------
IntersectCircleTangentDialog::~IntersectCircleTangentDialog() { delete ui; }
...
QString IntersectCircleTangentDialog::GetCircleRadius() const
{
    return qApp->translateVariables()->TryFormulaFromUser(
        ui->plainTextEditRadius->toPlainText(), qApp->Settings()->getOsSeparator());
}
...
void IntersectCircleTangentDialog::PointChanged()
{
    ...
    if (getCurrentObjectId(ui->comboBoxCircleCenter)
        == getCurrentObjectId(ui->comboBoxTangentPoint)) {
        flagError = false;
        color = errorColor;
    } else {
        flagError = true;
