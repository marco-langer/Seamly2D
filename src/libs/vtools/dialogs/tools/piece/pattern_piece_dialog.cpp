    ...
    , m_pieceLabelLines()
{
    ...
    connect(
        ...
        this,
        &PatternPieceDialog::pageChanged);
}
...
void PatternPieceDialog::pageChanged(QListWidgetItem* current, QListWidgetItem* previous)
{
    ...
    if (rowIndex == TabOrder::Labels || rowIndex == TabOrder::AnchorPoints
        || rowIndex == TabOrder::Grainline) {
        showAnchorPoints();
    } else {
...
//---------------------------------------------------------------------------------------------------------------------
VPiece PatternPieceDialog::GetPiece() const { return CreatePiece(); }
...
void PatternPieceDialog::SetPiece(const VPiece& piece)
{
    ...
    const QString width = qApp->translateVariables()->FormulaToUser(
        piece.getSeamAllowanceWidthFormula(), qApp->Settings()->getOsSeparator());
    ...
    ChangeCurrentData(
        ui->patternLabelBottomRightAnchor_ComboBox, m_oldGeom.bottomRightAnchorPoint());
    ...
    if (!prepare) {
        ...
        if (!applyAllowed) {
            ...
            if (p.GetPath().CountNodes() == 1) {
                emit ToolTip(
                    tr("Select main path objects clockwise, Use <b>SHIFT</b> to reverse curve "
                       "direction, "
...
void PatternPieceDialog::CheckState()
{
    ...
    ok_Button->setEnabled(
        flagName && flagFormula && flagMainPath && (flagGrainlineFormula || flagGrainlineAnchor)
    ...
    if (applyAllowed) {
        ...
        apply_Button->setEnabled(
            ok_Button->isEnabled());   // In case dialog does not have an apply button
    }
    ...
    if (flagMainPath) {
        ...
        if (!applyAllowed) {
            tooltip = tooltip + QLatin1String("  <b>") + tr("Press OK to create pattern piece")
                    + QLatin1String("</b>");
        }
 ...
 *         if event sent by an object other than the main path list widget pass the event on to the
 * parent.
 */
bool PatternPieceDialog::eventFilter(QObject* object, QEvent* event)
{
    if (QListWidget* list = qobject_cast<QListWidget*>(object)) {
        if (event->type() == QEvent::KeyPress) {
            ...
            if (ui->mainPath_ListWidget->count() <= 0 || row < 0
                || row >= ui->mainPath_ListWidget->count()) {
                return false;
            }
...
void PatternPieceDialog::pieceColorChanged()
{
    const QColor color = QColorDialog::getColor(
        Qt::white, this, tr("Select Color"), QColorDialog::DontUseNativeDialog);
...
{
    const int row = ui->mainPath_ListWidget->currentRow();
    if (ui->mainPath_ListWidget->count() == 0 || row == -1
        || row >= ui->mainPath_ListWidget->count()) {
        return;
    }
    ...
    if (rowNode.GetTypeTool() != Tool::NodePoint) {
        ...
        actionSlit = notchTypeMenu->addAction(
            QIcon("://icon/24x24/slit_notch.png"), tr("Slit") + QStringLiteral("\tShift + S"));
        actionTNotch = notchTypeMenu->addAction(
            QIcon("://icon/24x24/t_notch.png"), tr("TNotch") + QStringLiteral("\tShift + T"));
        actionUNotch = notchTypeMenu->addAction(
            QIcon("://icon/24x24/u_notch.png"), tr("UNotch") + QStringLiteral("\tShift + U"));
        actionVInternal = notchTypeMenu->addAction(
            QIcon("://icon/24x24/internal_v_notch.png"),
            tr("VInternal") + QStringLiteral("\tShift + I"));
        actionVExternal = notchTypeMenu->addAction(
            QIcon("://icon/24x24/external_v_notch.png"),
            tr("VExternal") + QStringLiteral("\tShift + E"));
        actionCastle = notchTypeMenu->addAction(
            QIcon("://icon/24x24/castle_notch.png"), tr("Castle") + QStringLiteral("\tShift + C"));
        actionDiamond = notchTypeMenu->addAction(
            QIcon("://icon/24x24/diamond_notch.png"),
            tr("Diamond") + QStringLiteral("\tShift + D"));

        QMenu* notchSubtypeMenu = notchMenu->addMenu(tr("Subtype"));
        actionStraightforward = notchSubtypeMenu->addAction(
            QIcon(), tr("Straightforward") + QStringLiteral("\tShift + F"));
        actionBisector =
            notchSubtypeMenu->addAction(QIcon(), tr("Bisector") + QStringLiteral("\tShift + B"));
        actionIntersection = notchSubtypeMenu->addAction(
...
void PatternPieceDialog::showCustomSAContextMenu(const QPoint& pos)
{
    ...
    if (ui->customSeamAllowance_ListWidget->count() == 0 || row == -1
        || row >= ui->customSeamAllowance_ListWidget->count()) {
        return;
    }
    ...
    if (selectedAction == actionDelete) {
        ...
        connect(
            ...
            this,
            &PatternPieceDialog::pathDialogClosed);
        m_dialog->show();
    }
}
...
void PatternPieceDialog::showAnchorsContextMenu(const QPoint& pos)
{
    ...
    if (ui->anchorPoints_ListWidget->count() == 0 || row == -1
        || row >= ui->anchorPoints_ListWidget->count()) {
        return;
    }
...
void PatternPieceDialog::nodeChanged(int index)
{
    ...
    if (index != -1) {
        ...
        const int nodeIndex =
            piece.GetPath().indexOfNode(ui->nodes_ComboBox->currentData().toUInt());
        if (nodeIndex != -1) {
            ...
            w1Formula = qApp->translateVariables()->FormulaToUser(
                w1Formula, qApp->Settings()->getOsSeparator());
            ...
            w2Formula = qApp->translateVariables()->FormulaToUser(
                w2Formula, qApp->Settings()->getOsSeparator());
            ...
            const int index =
                ui->angle_ComboBox->findData(static_cast<unsigned char>(node.GetAngleType()));
            if (index != -1) {
...
void PatternPieceDialog::notchChanged(int index)
{
    ...
    if (index != -1) {
        const VPiece piece = CreatePiece();
        const int nodeIndex =
            piece.GetPath().indexOfNode(ui->notches_ComboBox->currentData().toUInt());
        if (nodeIndex != -1) {
            const VPieceNode& node = piece.GetPath().at(nodeIndex);
            ...
            case NotchType::Slit:
                ...
                break;
            case NotchType::TNotch: ui->tNotch_RadioButton->setChecked(true); break;
            case NotchType::UNotch: ui->uNotch_RadioButton->setChecked(true); break;
            case NotchType::VInternal: ui->vInternalNotch_RadioButton->setChecked(true); break;
...
void PatternPieceDialog::nodeAngleChanged(int index)
{
    ...
    if (i != -1 && index != -1) {
        ...
        if (rowItem) {
            VPieceNode rowNode = qvariant_cast<VPieceNode>(rowItem->data(Qt::UserRole));
            rowNode.SetAngleType(
                static_cast<PieceNodeAngle>(ui->angle_ComboBox->currentData().toUInt()));
...
{
    const QString allowance = qApp->translateVariables()->FormulaToUser(
        currentSeamAllowance, qApp->Settings()->getOsSeparator());
...
void PatternPieceDialog::customSeamAllowanceChanged(int row)
{
    ...
    {
        const int index =
            ui->comboBoxIncludeType->findData(static_cast<unsigned char>(record.includeType));
        if (index != -1) {
...
void PatternPieceDialog::pathDialogClosed(int result)
{
    if (result == QDialog::Accepted) {
        ...
        try {
            ...
            qCritical(
                "%s\n\n%s\n\n%s",
                qUtf8Printable(tr("Error. Can't save piece path.")),
...
void PatternPieceDialog::resetNotchAngle() { ui->notchAngle_DoubleSpinBox->setValue(0.0); }
...
void PatternPieceDialog::updateGrainlineValues()
{
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        try {
            ...
            !flagGrainlineAnchor ? ChangeColor(labelText, Qt::red)
                                 : ChangeColor(labelText, okColor);
            formulasOK[i] = false;
...
{
    QPlainTextEdit* lineEdit[3] = { ui->pieceLabelWidthFormula_LineEdit,
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        try {
            ...
            formulaValueStr = tr("Error");
            !flagPieceLabelAnchor ? ChangeColor(labelText, Qt::red)
                                  : ChangeColor(labelText, okColor);
            formulasOK[i] = false;
            labelValue->setToolTip(tr("Parser error: %1").arg(error.GetMsg()));
        }
...
{
    QPlainTextEdit* lineEdit[3] = { ui->patternLabelWidthFormula_LineEdit,
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        try {
            ...
            formulaValueStr = tr("Error");
            !flagPatternLabelAnchor ? ChangeColor(labelText, Qt::red)
                                    : ChangeColor(labelText, okColor);
            formulasOK[i] = false;
            labelValue->setToolTip(tr("Parser error: %1").arg(error.GetMsg()));
        }
...
void PatternPieceDialog::editGrainlineFormula()
{
    ...
    dialog.SetFormula(qApp->translateVariables()->TryFormulaFromUser(
        labelFormula->toPlainText(), qApp->Settings()->getOsSeparator()));
...
void PatternPieceDialog::beforeWidthChanged()
{
    ...
    ValFormulaChanged(
        flagBeforeFormula, ui->beforeWidthFormula_PlainTextEdit, m_timerWidthBefore, postfix);
}
...
void PatternPieceDialog::afterWidthChanged()
{
    ...
    ValFormulaChanged(
        flagAfterFormula, ui->afterWidthFormula_PlainTextEdit, m_timerWidthAfter, postfix);
}
...
void PatternPieceDialog::pieceLabelAnchorChanged()
{
    ...
    if (topAnchorId != NULL_ID && bottomAnchorId != NULL_ID && topAnchorId != bottomAnchorId) {
        ...
        if (flagPatternLabelAnchor) {
            clearErrorText(TabOrder::Labels, tr("Labels "));
            QIcon icon(":/icon/32x32/piece_label.png");
            ui->labels_TabWidget->setTabIcon(
                ui->labels_TabWidget->indexOf(ui->pieceLabel_Tab), icon);
        }
    } else {
...
void PatternPieceDialog::patternLabelAnchorChanged()
{
    ...
    if (topAnchorId != NULL_ID && bottomAnchorId != NULL_ID && topAnchorId != bottomAnchorId) {
        ...
        if (flagPieceLabelAnchor) {
            ...
            QIcon icon(":/icon/32x32/pattern_label.png");
            ui->labels_TabWidget->setTabIcon(
                ui->labels_TabWidget->indexOf(ui->patternLabel_Tab), icon);
        }
    } else {
...
{
    QVector<VLabelTemplateLine> patternLabelLines =
        qApp->getCurrentDocument()->getPatternLabelTemplate();
...
VPiece PatternPieceDialog::CreatePiece() const
{
    ...
    piece.SetMx(m_mx);
    piece.SetMy(m_my);
    piece.setSeamAllowanceWidthFormula(
        getFormulaFromUser(ui->widthFormula_PlainTextEdit), m_saWidth);
    ...
    piece.GetPatternPieceData().SetLabelWidth(
        getFormulaFromUser(ui->pieceLabelWidthFormula_LineEdit));
    piece.GetPatternPieceData().SetLabelHeight(
        getFormulaFromUser(ui->pieceLabelHeightFormula_LineEdit));
    piece.GetPatternPieceData().SetRotation(
        getFormulaFromUser(ui->pieceLabelAngleFormula_LineEdit));
    piece.GetPatternPieceData().SetFontSize(m_oldData.getFontSize());
    piece.GetPatternPieceData().setCenterAnchorPoint(
        getCurrentObjectId(ui->pieceLabelCenterAnchor_ComboBox));
    piece.GetPatternPieceData().setTopLeftAnchorPoint(
        getCurrentObjectId(ui->pieceLabelTopLeftAnchor_ComboBox));
    piece.GetPatternPieceData().setBottomRightAnchorPoint(
        getCurrentObjectId(ui->pieceLabelBottomRightAnchor_ComboBox));
    ...
    piece.GetPatternInfo().SetLabelHeight(
        getFormulaFromUser(ui->patternLabelHeightFormula_LineEdit));
    piece.GetPatternInfo().SetRotation(getFormulaFromUser(ui->patternLabelAngleFormula_LineEdit));
    piece.GetPatternInfo().setCenterAnchorPoint(
        getCurrentObjectId(ui->patternLabelCenterAnchor_ComboBox));
    piece.GetPatternInfo().setTopLeftAnchorPoint(
        getCurrentObjectId(ui->patternLabelTopLeftAnchor_ComboBox));
    piece.GetPatternInfo().setBottomRightAnchorPoint(
        getCurrentObjectId(ui->patternLabelBottomRightAnchor_ComboBox));
    ...
    piece.GetGrainlineGeometry().SetArrowType(
        static_cast<ArrowType>(ui->arrow_ComboBox->currentIndex()));
    piece.GetGrainlineGeometry().setCenterAnchorPoint(
        getCurrentObjectId(ui->grainlineCenterAnchor_ComboBox));
    piece.GetGrainlineGeometry().setTopAnchorPoint(
        getCurrentObjectId(ui->grainlineTopAnchor_ComboBox));
    piece.GetGrainlineGeometry().setBottomAnchorPoint(
        getCurrentObjectId(ui->grainlineBottomAnchor_ComboBox));
...
void PatternPieceDialog::newCustomSeamAllowance(const CustomSARecord& record)
{
    if (record.path > NULL_ID) {
        ...
        ui->customSeamAllowance_ListWidget->setCurrentRow(
...
//---------------------------------------------------------------------------------------------------------------------
bool PatternPieceDialog::isInLayout() const { return m_inLayout; }

//---------------------------------------------------------------------------------------------------------------------
void PatternPieceDialog::setInLayout(const bool& state) { m_inLayout = state; }

//---------------------------------------------------------------------------------------------------------------------
bool PatternPieceDialog::getPieceLock() const { return m_isLocked; }

//---------------------------------------------------------------------------------------------------------------------
void PatternPieceDialog::setPieceLock(const bool& state) { m_isLocked = state; }
...
void PatternPieceDialog::initializeMainPathTab()
{
    ...
    connect(
        ...
        this,
        &PatternPieceDialog::setMoveExclusions);
...
void PatternPieceDialog::initializeSeamAllowanceTab()
{
    ...
    connect(
        ui->seams_CheckBox, &QCheckBox::toggled, this, &PatternPieceDialog::enableSeamAllowance);
    connect(ui->builtIn_CheckBox, &QCheckBox::toggled, this, &PatternPieceDialog::enableBuiltIn);

    // Initialize the default seam allowance, convert the value if app unit is different than
    // pattern unit
    m_saWidth = UnitConvertor(
        qApp->Settings()->GetDefaultSeamAllowance(),
        StrToUnits(qApp->Settings()->GetUnit()),
        qApp->patternUnit());
    ...
    connect(
        ui->beforeDefault_PushButton,
        &QPushButton::clicked,
        this,
        &PatternPieceDialog::enableDefaultBeforeButton);
    connect(
        ui->afterDefault_PushButton,
        &QPushButton::clicked,
        this,
        &PatternPieceDialog::enableDefaultAfterButton);
    ...
    connect(
        ...
        static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged),
        this,
        &PatternPieceDialog::customSAStartPointChanged);
    connect(
    ...
    connect(
        ...
        static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged),
        this,
        &PatternPieceDialog::customSAIncludeTypeChanged);

    connect(
        ui->toolButtonExprWidth,
        &QPushButton::clicked,
        this,
        &PatternPieceDialog::editDefaultSeamAllowanceWidth);
    connect(
        ui->beforeExpr_ToolButton,
        &QPushButton::clicked,
        this,
        &PatternPieceDialog::editBeforeSeamAllowanceWidth);
    connect(
        ui->afterExpr_ToolButton,
        &QPushButton::clicked,
        this,
        &PatternPieceDialog::editAfterSeamAllowanceWidth);

    connect(
        ui->widthFormula_PlainTextEdit,
...
void PatternPieceDialog::initializeSAIncludeType()
{
    ...
    ui->comboBoxIncludeType->addItem(
        tr("main path"), static_cast<unsigned char>(PiecePathIncludeType::AsMainPath));
    ui->comboBoxIncludeType->addItem(
        tr("custom seam allowance"), static_cast<unsigned char>(PiecePathIncludeType::AsCustomSA));
}
...
void PatternPieceDialog::initializePropertiesTab()
{
    ...
    connect(
        ui->pieceName_LineEdit,
        &QLineEdit::textChanged,
        this,
        &PatternPieceDialog::pieceNameChanged);
    connect(
        ui->colorPicker_ToolButton,
        &QToolButton::clicked,
        this,
        &PatternPieceDialog::pieceColorChanged);
}
...
void PatternPieceDialog::initializeLabelsTab()
{
    ...
    ui->pieceLabelWidthFormula_LineEdit->setPlainText(
        QString::number(qApp->Settings()->getDefaultLabelWidth()));
    ui->pieceLabelHeightFormula_LineEdit->setPlainText(
        QString::number(qApp->Settings()->getDefaultLabelHeight()));

    connect(
        ui->pieceLabel_GroupBox, &QGroupBox::toggled, this, &PatternPieceDialog::enabledPieceLabel);
    ...
    connect(
        ui->pieceLabelTopLeftAnchor_ComboBox,
        &QComboBox::currentTextChanged,
        this,
        &PatternPieceDialog::pieceLabelAnchorChanged);

    connect(
        ui->pieceLabelBottomRightAnchor_ComboBox,
        &QComboBox::currentTextChanged,
        this,
        &PatternPieceDialog::pieceLabelAnchorChanged);
    ...
    connect(
        ...
        &QPushButton::clicked,
        this,
        &PatternPieceDialog::editPieceLabelFormula);
    connect(
    ...
    connect(
        ui->editPieceLabel_PushButton,
        &QPushButton::clicked,
        this,
        &PatternPieceDialog::editPieceLabel);
    ...
    connect(
        ui->patternLabel_GroupBox,
        &QGroupBox::toggled,
        this,
        &PatternPieceDialog::enabledPatternLabel);
    ...
    connect(
        ui->patternLabelTopLeftAnchor_ComboBox,
        &QComboBox::currentTextChanged,
        this,
        &PatternPieceDialog::patternLabelAnchorChanged);

    connect(
        ui->patternLabelBottomRightAnchor_ComboBox,
        &QComboBox::currentTextChanged,
        this,
        &PatternPieceDialog::patternLabelAnchorChanged);
    ...
    connect(
        ...
        &QPushButton::clicked,
        this,
        &PatternPieceDialog::editPatternLabelFormula);
    connect(
    ...
    connect(
        ui->editPatternLabel_PushButton,
        &QPushButton::clicked,
        this,
        &PatternPieceDialog::editPatternLabel);
...
void PatternPieceDialog::initializeGrainlineTab()
{
    ...
    ui->lengthFormula_LineEdit->setPlainText(
        QString::number(qApp->Settings()->getDefaultGrainlineLength()));

    connect(
        ui->grainline_GroupBox, &QGroupBox::toggled, this, &PatternPieceDialog::enabledGrainline);
    connect(
        ui->rotation_PushButton,
        &QPushButton::clicked,
        this,
        &PatternPieceDialog::editGrainlineFormula);
    connect(
    ...
    connect(
        ui->grainlineTopAnchor_ComboBox,
        &QComboBox::currentTextChanged,
        this,
        &PatternPieceDialog::grainlineAnchorChanged);

    connect(
        ui->grainlineBottomAnchor_ComboBox,
        &QComboBox::currentTextChanged,
        this,
        &PatternPieceDialog::grainlineAnchorChanged);
}
...
void PatternPieceDialog::initializeNotchesTab()
{
    ...
    connect(
        ...
        static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged),
        this,
        &PatternPieceDialog::notchChanged);

    connect(
        ui->notchType_ButtonGroup,
        &QButtonGroup::idClicked,
        this,
        &PatternPieceDialog::notchTypeChanged);

    connect(
        ui->notchSubType_ButtonGroup,
        &QButtonGroup::idClicked,
        this,
        &PatternPieceDialog::notchSubTypeChanged);

    connect(
        ui->showNotch_CheckBox,
        &QCheckBox::stateChanged,
        this,
        &PatternPieceDialog::showNotchChanged);

    connect(
        ui->showSeamlineNotch_CheckBox,
        &QCheckBox::stateChanged,
        this,
        &PatternPieceDialog::showSeamlineNotchChanged);

    connect(
        ui->notchLength_DoubleSpinBox,
        QOverload<double>::of(&QDoubleSpinBox::valueChanged),
        this,
        &PatternPieceDialog::notchLengthChanged);

    connect(
        ui->resetLength_PushButton,
        &QPushButton::clicked,
        this,
        &PatternPieceDialog::resetNotchLength);

    connect(
        ui->notchWidth_DoubleSpinBox,
        QOverload<double>::of(&QDoubleSpinBox::valueChanged),
        this,
        &PatternPieceDialog::notchWidthChanged);

    connect(
        ui->resetWidth_PushButton,
        &QPushButton::clicked,
        this,
        &PatternPieceDialog::resetNotchWidth);

    connect(
        ui->notchAngle_DoubleSpinBox,
        QOverload<double>::of(&QDoubleSpinBox::valueChanged),
        this,
        &PatternPieceDialog::notchAngleChanged);

    connect(
        ui->resetAngle_PushButton,
        &QPushButton::clicked,
        this,
        &PatternPieceDialog::resetNotchAngle);

    connect(
        ui->notchCount_SpinBox,
        QOverload<int>::of(&QSpinBox::valueChanged),
        this,
        &PatternPieceDialog::notchCountChanged);
}
...
QString PatternPieceDialog::getSeamAllowanceWidthFormula() const
{
    ...
    width.replace("\n", " ");
    return qApp->translateVariables()->TryFormulaFromUser(
        width, qApp->Settings()->getOsSeparator());
}
...
void PatternPieceDialog::setGrainlineAngle(QString angleFormula)
{
    ...
    const QString formula =
        qApp->translateVariables()->FormulaToUser(angleFormula, qApp->Settings()->getOsSeparator());
...
void PatternPieceDialog::setGrainlineLength(QString lengthFormula)
{
    ...
    const QString formula = qApp->translateVariables()->FormulaToUser(
...
void PatternPieceDialog::setPieceLabelWidth(QString widthFormula)
{
    ...
    const QString formula =
        qApp->translateVariables()->FormulaToUser(widthFormula, qApp->Settings()->getOsSeparator());
...
void PatternPieceDialog::setPieceLabelHeight(QString heightFormula)
{
    ...
    const QString formula = qApp->translateVariables()->FormulaToUser(
        heightFormula, qApp->Settings()->getOsSeparator());
 ...
 */
void PatternPieceDialog::setNotch(
    QListWidgetItem* rowItem, bool isNotch, NotchType notchType, NotchSubType notchSubType)
{
    VPieceNode rowNode = qvariant_cast<VPieceNode>(rowItem->data(Qt::UserRole));
    if (rowNode.GetTypeTool() == Tool::NodePoint) {
...
qreal PatternPieceDialog::getFormulaValue(QPlainTextEdit* text) const
{
    ...
    formula.replace("\n", " ");
    formula =
        qApp->translateVariables()->FormulaFromUser(formula, qApp->Settings()->getOsSeparator());
    return ToPixel(
        calculation.EvalFormula(data->DataVariables(), formula), *data->GetPatternUnit());
}
