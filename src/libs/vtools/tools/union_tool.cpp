...
//---------------------------------------------------------------------------------------------------------------------
VPiecePath GetPiece1MainPath(VAbstractPattern* doc, quint32 id) { return GetPiecePath(1, doc, id); }

//---------------------------------------------------------------------------------------------------------------------
VPiecePath GetPiece2MainPath(VAbstractPattern* doc, quint32 id) { return GetPiecePath(2, doc, id); }
    ...
    qreal angle)
{
    ...
    if (not qFuzzyIsNull(dx) || not qFuzzyIsNull(dy) || pRotate != NULL_ID) {
        BiasRotatePoint(
            ...
            dx,
            dy,
            static_cast<QPointF>(*initData.data->GeometricObject<VPointF>(pRotate)),
            angle);
    }

    auto point1{ std::make_unique<VPointF>(*point) };

    const quint32 objectId = initData.data->AddGObject(std::move(point));
    children.append(objectId);
    point1->setMode(Draw::Modeling);
    const quint32 id = initData.data->AddGObject(std::move(point1));
    VNodePoint::Create(
        initData.doc,
        initData.data,
        initData.scene,
    ...
    qreal angle)
{
    ...
    children.append(objectId);
    point1->setMode(Draw::Modeling);
    const quint32 anchorId = initData.data->AddGObject(std::move(point1));
    ...
    qreal angle)
{
    ...
    VPointF* tmpCenter = center.get();
    const quint32 idCenter = initData.data->AddGObject(std::move(center));
    Q_UNUSED(idCenter)
    auto arc1{ std::make_unique<VArc>(
        *tmpCenter,
        arc.GetRadius(),
    ...
    const quint32 objectId = initData.data->AddGObject(std::move(arc1));
    children.append(objectId);

    arc2->setMode(Draw::Modeling);
    const quint32 id = initData.data->AddGObject(std::move(arc2));
    ...
    qreal angle)
{
    ...
    center->setMode(Draw::Modeling);
    VPointF* tmpCenter = center.get();
    quint32 idCenter = initData.data->AddGObject(std::move(center));
    Q_UNUSED(idCenter)
    ...
    qreal angle)
{
    ...
    VPointF p1{ spline.GetP1() };
    VPointF p2{ spline.GetP2() };
    VPointF p3{ spline.GetP3() };
    VPointF p4{ spline.GetP4() };
    ...
    qreal angle)
{
    ...
    for (qint32 i = 1; i <= splinePath.CountSubSpl(); ++i) {
        ...
        if (i == 1) {
            ...
            path->append(VSplinePoint(
                ...
                angle1F,
                spl.GetStartAngle(),
                spl.GetStartAngleFormula(),
                0,
                "0",
                spline.GetC1Length(),
                spline.GetC1LengthFormula()));
        }
    ...
    const quint32 objectId = initData.data->AddGObject(std::move(path));
    children.append(objectId);

    path1->setMode(Draw::Modeling);
    const quint32 id = initData.data->AddGObject(std::move(path1));

    VNodeSplinePath::Create(
        initData.doc,
        initData.data,
        id,
        objectId,
...
void FindIndexJ(qint32 piece2Points, const VPiecePath& piece2Path, quint32 piece2_Index, qint32& j)
{
    if (piece2Points == 0) {
        ...
        SCASSERT(k != -1)
        if (k
            == removedPiece.CountNodes() - 1) {   // We have last node in piece, we wil begin from 0
            j = 0;
        } else {   // Continue from next node
            j = k + 1;
        }
    }
}
    ...
    qreal angle)
{
    ...
    if (not qFuzzyIsNull(dx) || not qFuzzyIsNull(dy) || pRotate != NULL_ID) {
        BiasRotatePoint(
            ...
            dx,
            dy,
            static_cast<QPointF>(*data->GeometricObject<VPointF>(pRotate)),
            angle);
    }
    data->UpdateGObject(TakeNextId(children), std::move(point));
}
    ...
    qreal angle)
{
    ...
    auto arc1{ std::make_unique<VArc>(
        ...
        QString().setNum(l2.angle())) };
    arc1->setMode(Draw::Modeling);
    data->UpdateGObject(TakeNextId(children), std::move(arc1));
}

//---------------------------------------------------------------------------------------------------------------------
void UpdateNodeElArc(
    VContainer* data,
    const VPieceNode& node,
    QVector<quint32>& children,
    qreal dx,
    ...
    qreal angle)
{
    ...
    data->UpdateGObject(TakeNextId(children), std::move(spl));
}

//---------------------------------------------------------------------------------------------------------------------
void UpdateNodeSplinePath(
    VContainer* data,
    const VPieceNode& node,
    QVector<quint32>& children,
    qreal dx,
    ...
    qreal angle)
{
    ...
    data->UpdateGObject(TakeNextId(children), std::move(path));
}
    ...
    qreal angle)
{
    ...
    for (int i = 0; i < d.GetCustomSARecords().size(); ++i) {
        ...
        for (int i = 0; i < path.CountNodes(); ++i) {
            AddNodeToNewPath(
                initData, newPath, path.at(i), id, nodeChildren, blockName, dx, dy, pRotate, angle);
        }
...
//---------------------------------------------------------------------------------------------------------------------
void CreateUnitedInternalPaths(
    VPiece& newPiece,
    const VPiece& piece1,
    const VPiece& piece2,
    quint32 id,
    ...
    qreal angle)
{
    ...
    createUnionInternalPaths(
        newPiece, piece2, children, id, blockName, initData, dx, dy, pRotate, angle);

    SaveInternalPathsChildren(initData.doc, id, children);
}

//---------------------------------------------------------------------------------------------------------------------
void createUnionAnchors(
    VPiece& newPiece,
    const VPiece& d,
    QVector<quint32>& children,
    quint32 idTool,
    ...
    qreal angle)
{
    ...
    for (int i = 0; i < d.getAnchors().size(); ++i) {
        const quint32 id = AddAnchorPoint(
            d.getAnchors().at(i), initData, idTool, children, blockName, dx, dy, pRotate, angle);
...
//---------------------------------------------------------------------------------------------------------------------
void UpdateUnitedNodes(
    quint32 id, const UnionToolInitData& initData, qreal dx, qreal dy, quint32 pRotate, qreal angle)
{
    ...
    if (not children.isEmpty()) {
        ...
        if (children.size() == piece1NodeCount + piece2NodeCount - 1) {
            qint32 piece2Points =
                0;   // Keeps number points the second piece, what we have already added.
            qint32 i = 0;
            ...
            do {
                ...
                if (i > indexOfNode && piece2Points < piece2NodeCount - 1) {
                    ...
                    do {
                        ...
                        UpdatePathNode(
                            initData.data, piece2REPath.at(j), children, dx, dy, pRotate, angle);
                        ++piece2Points;
                        ++j;
                    } while (piece2Points < piece2NodeCount - 1);
                }
...
{
    createUnionPaths(
        initData, dx, dy, pRotate, angle, records, GetInternalPathsChildren(initData.doc, id));
}

//---------------------------------------------------------------------------------------------------------------------
void updateUnionAnchors(
    quint32 id,
    const UnionToolInitData& initData,
    qreal dx,
    qreal dy,
    ...
    const QVector<quint32>& records)
{
    ...
    for (int i = 0; i < records.size(); ++i) {
        auto point{ std::make_unique<VPointF>(
            *initData.data->GeometricObject<VPointF>(records.at(i))) };
        point->setMode(Draw::Modeling);
        if (not qFuzzyIsNull(dx) || not qFuzzyIsNull(dy) || pRotate != NULL_ID) {
            BiasRotatePoint(
                point.get(),
        ...
        initData.data->UpdateGObject(TakeNextId(children), std::move(point));
    }
}
    ...
    quint32 id, const UnionToolInitData& initData, qreal dx, qreal dy, quint32 pRotate, qreal angle)
{
    ...
    newPiece.GetPatternInfo().SetLabelWidth(
        QString::number(qApp->Settings()->getDefaultLabelWidth()));
    newPiece.GetPatternInfo().SetLabelHeight(
    ...
    newPiece.GetPatternPieceData().SetLabelWidth(
        QString::number(qApp->Settings()->getDefaultLabelWidth()));
    ...
    newPiece.GetGrainlineGeometry().SetLength(
    ...
    auto removeUnionPiece = [initData](quint32 id) {
        PatternPieceTool* pieceTool =
            qobject_cast<PatternPieceTool*>(VAbstractPattern::getTool(id));
    ...
    quint32 id, const UnionToolInitData& initData, qreal dx, qreal dy, quint32 pRotate, qreal angle)
{
    ...
    updateUnionInternalPaths(
        id, initData, dx, dy, pRotate, angle, GetPiece2InternalPaths(initData.doc, id));
...
//---------------------------------------------------------------------------------------------------------------------
QString UnionTool::getTagName() const { return VAbstractPattern::TagTools; }

//---------------------------------------------------------------------------------------------------------------------
void UnionTool::ShowVisualization(bool show) { Q_UNUSED(show) }
...
UnionTool* UnionTool::Create(const quint32 _id, const UnionToolInitData& initData)
{
    ...
    if (initData.parse == Document::FullParse) {
        ...
        // Unfortunatelly doc will destroy all objects only in the end, but we should delete them
        // before each FullParse
...
void UnionTool::AddToModeling(const QDomElement& domElement)
{
    ...
    QDomElement modeling =
        doc->getDraftBlockElement(blockName).firstChildElement(VAbstractPattern::TagModeling);
                ...
                if (not element.isNull()) {
                    ...
                    case 2:   // PatternPieceTool::TagIPaths
                        list += ReferenceObjects(
                            element, PatternPieceTool::TagRecord, VAbstractPattern::AttrPath);
                        break;
                    case 3:   // PatternPieceTool::TagAnchors
                    {
                        ...
                        for (qint32 i = 0; i < children.size(); ++i) {
                            ...
                            if (not record.isNull()
                                && record.tagName() == PatternPieceTool::TagRecord) {
...
//---------------------------------------------------------------------------------------------------------------------
QVector<quint32> UnionTool::ReferenceObjects(
    const QDomElement& root, const QString& tag, const QString& attribute) const
{
