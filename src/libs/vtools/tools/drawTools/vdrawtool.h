 ...
 */
void VDrawTool::ContextMenu(
    QGraphicsSceneContextMenuEvent* event,
    quint32 itemId,
    const RemoveOption& showRemove,
    const Referens& ref)
{
    ...
    QMap<quint32, QString> groupsNotContainingItem =
        doc->getGroupsContainingItem(this->getId(), itemId, false);
    QActionGroup* actionAddGroupMenu = new QActionGroup(this);

    if (not groupsNotContainingItem.empty()) {
        QMenu* menuAddGroupItem =
            menu.addMenu(QIcon("://icon/32x32/add.png"), tr("Add Group Object"));
    ...
    QMap<quint32, QString> groupsContainingItem =
        doc->getGroupsContainingItem(this->getId(), itemId, true);
    QActionGroup* actionDeleteGroupMenu = new QActionGroup(this);

    if (not groupsContainingItem.empty()) {
        QMenu* menuRemoveGroupItem =
            menu.addMenu(QIcon("://icon/32x32/remove.png"), tr("Remove Group Object"));
    ...
    if (selectedAction == actionOption) {
        ...
        connect(
            m_dialog.data(), &DialogTool::DialogApplied, this, &VDrawTool::FullUpdateFromGuiApply);
        ...
        SCASSERT(window != nullptr)
        {
            ...
            connect(
                command, &AddGroupItem::updateGroups, window, &VAbstractMainWindow::updateGroups);
...
QString VDrawTool::ObjectName(quint32 id) const
{
    try {
        ...
        qCWarning(
            ...
            qUtf8Printable(QString().setNum(id)),
