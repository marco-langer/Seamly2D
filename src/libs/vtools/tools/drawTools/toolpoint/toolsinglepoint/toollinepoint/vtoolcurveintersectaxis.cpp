...
//---------------------------------------------------------------------------------------------------------------------
VToolCurveIntersectAxis* VToolCurveIntersectAxis::Create(
    ...
    VContainer* data)
{
    ...
    QSharedPointer<DialogCurveIntersectAxis> dialogTool =
        dialog.objectCast<DialogCurveIntersectAxis>();
    SCASSERT(not dialogTool.isNull())
    ...
    VToolCurveIntersectAxis* point = Create(
        0,
        pointName,
        lineType,
        lineWeight,
    ...
    const Source& typeCreation)
{
    ...
    if (parse == Document::FullParse) {
        ...
        VToolCurveIntersectAxis* point = new VToolCurveIntersectAxis(
            doc,
            data,
            id,
            lineType,
...
//---------------------------------------------------------------------------------------------------------------------
quint32 VToolCurveIntersectAxis::getCurveId() const { return curveId; }
...
//---------------------------------------------------------------------------------------------------------------------
template <class Item>
void VToolCurveIntersectAxis::InitArc(
    VContainer* data, qreal segLength, const VPointF* p, quint32 curveId)
{
...
{
    switch (curveType) {
    case GOType::EllipticalArc: InitArc<VEllipticalArc>(data, segLength, p, curveId); break;
    case GOType::Arc: InitArc<VArc>(data, segLength, p, curveId); break;
