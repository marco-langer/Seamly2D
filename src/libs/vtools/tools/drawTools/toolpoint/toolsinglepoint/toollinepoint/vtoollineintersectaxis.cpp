...
VToolLineIntersectAxis::VToolLineIntersectAxis(
    ...
    : VToolLinePoint(
        doc, data, id, lineType, lineWeight, lineColor, QString(), basePointId, 0, parent)
...
void VToolLineIntersectAxis::setDialog()
{
    ...
    QSharedPointer<DialogLineIntersectAxis> dialogTool =
        m_dialog.objectCast<DialogLineIntersectAxis>();
...
//---------------------------------------------------------------------------------------------------------------------
VToolLineIntersectAxis* VToolLineIntersectAxis::Create(
    ...
    VContainer* data)
{
    ...
    QSharedPointer<DialogLineIntersectAxis> dialogTool =
        dialog.objectCast<DialogLineIntersectAxis>();
    SCASSERT(not dialogTool.isNull())
    ...
    VToolLineIntersectAxis* point = Create(
        0,
        pointName,
        lineType,
        lineWeight,
    ...
    const Source& typeCreation)
{
    ...
    QLineF axis{ static_cast<QPointF>(basePoint), QPointF(basePoint.x() + 100, basePoint.y()) };
    ...
    const QLineF line{ static_cast<QPointF>(firstPoint), static_cast<QPointF>(secondPoint) };
...
//---------------------------------------------------------------------------------------------------------------------
quint32 VToolLineIntersectAxis::GetFirstPointId() const { return firstPointId; }
...
//---------------------------------------------------------------------------------------------------------------------
quint32 VToolLineIntersectAxis::GetSecondPointId() const { return secondPointId; }
...
//---------------------------------------------------------------------------------------------------------------------
void VToolLineIntersectAxis::SaveOptions(QDomElement& tag, const VGObject* obj)
{
...
QString VToolLineIntersectAxis::makeToolTip() const
{
    ...
    const auto& firstPoint{ *VAbstractTool::data.GeometricObject<VPointF>(firstPointId) };
