    ...
    const Source& typeCreation)
{
    ...
    if (parse == Document::FullParse) {
        ...
        VToolLine* line = new VToolLine(
            doc, data, id, firstPoint, secondPoint, lineType, lineWeight, lineColor, typeCreation);
        scene->addItem(line);
        InitDrawToolConnections(scene, line);
        connect(
            scene, &VMainGraphicsScene::EnableLineItemSelection, line, &VToolLine::AllowSelecting);
...
//---------------------------------------------------------------------------------------------------------------------
QString VToolLine::getTagName() const { return VAbstractPattern::TagLine; }

//---------------------------------------------------------------------------------------------------------------------
void VToolLine::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
    // const qreal width = scaleWidth(m_isHovered ? widthMainLine : widthHairLine,
    // sceneScale(scene()));
    ...
    setPen(QPen(
        correctColor(this, doc->useGroupColor(m_id, lineColor)),
        width,
        lineTypeToPenStyle(doc->useGroupLineType(m_id, m_lineType))));
 ...
 */
void VToolLine::ShowTool(quint32 id, bool enable) { ShowItem(this, id, enable); }
...
//---------------------------------------------------------------------------------------------------------------------
void VToolLine::AllowHover(bool enabled) { setAcceptHoverEvents(enabled); }

//---------------------------------------------------------------------------------------------------------------------
void VToolLine::AllowSelecting(bool enabled) { setFlag(QGraphicsItem::ItemIsSelectable, enabled); }
 ...
 * @param event context menu event.
 */
void VToolLine::contextMenuEvent(QGraphicsSceneContextMenuEvent* event) { showContextMenu(event); }
...
{
    switch (event->key()) {
    case Qt::Key_Delete: try { deleteTool();
        } catch (const VExceptionToolWasDeleted& error) {
            Q_UNUSED(error)
            return;   // Leave this method immediately!!!
        }
...
//---------------------------------------------------------------------------------------------------------------------
quint32 VToolLine::GetSecondPoint() const { return secondPoint; }
...
//---------------------------------------------------------------------------------------------------------------------
QString VToolLine::getLineColor() const { return lineColor; }
...
//---------------------------------------------------------------------------------------------------------------------
quint32 VToolLine::GetFirstPoint() const { return firstPoint; }
