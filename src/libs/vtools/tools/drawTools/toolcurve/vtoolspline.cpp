    ...
    , oldPosition()
{
    ...
    auto* controlPoint1 = new VControlPointSpline(
        1,
    ...
    connect(
        controlPoint1,
        &VControlPointSpline::ControlPointChangePosition,
        this,
        &VToolSpline::ControlPointChangePosition);
    connect(
        this, &VToolSpline::setEnabledPoint, controlPoint1, &VControlPointSpline::setEnabledPoint);
    connect(
        controlPoint1, &VControlPointSpline::showContextMenu, this, &VToolSpline::contextMenuEvent);
    ...
    auto* controlPoint2 = new VControlPointSpline(
        1,
    ...
    connect(
        ...
        &VToolSpline::ControlPointChangePosition);
    connect(
        this, &VToolSpline::setEnabledPoint, controlPoint2, &VControlPointSpline::setEnabledPoint);
    connect(
        controlPoint2, &VControlPointSpline::showContextMenu, this, &VToolSpline::contextMenuEvent);
...
{
    return *VAbstractTool::data.GeometricObject<VSpline>(m_id);
}
...
//---------------------------------------------------------------------------------------------------------------------
void VToolSpline::ShowVisualization(bool show) { ShowToolVisualization<VisToolSpline>(show); }
 ...
 */
void VToolSpline::ControlPointChangePosition(
    const qint32& indexSpline, const SplinePointPosition& position, const QPointF& pos)
{
    ...
    auto* moveSpl = new MoveSpline{ doc, &spline, spl, m_id };
    connect(moveSpl, &MoveSpline::NeedLiteParsing, doc, &VAbstractPattern::LiteParseTree);
    qApp->getUndoStack()->push(moveSpl);
}

//---------------------------------------------------------------------------------------------------------------------
void VToolSpline::EnableToolMove(bool move) { this->setFlag(QGraphicsItem::ItemIsMovable, move); }
...
//---------------------------------------------------------------------------------------------------------------------
void VToolSpline::SaveOptions(QDomElement& tag, const VGObject* obj)
{
    VAbstractSpline::SaveOptions(tag, obj);

    const auto* spl{ dynamic_cast<const VSpline*>(obj) };
    SCASSERT(spl)
...
{
    if (IsMovable()) {
        // Don't need check if left mouse button was pressed. According to the Qt documentation "If
        // you do receive this event, you can be certain that this item also received a mouse press
        // event, and that this item is the current mouse grabber.".
        ...
        VSpline spl{ spline.GetP1(), p2, p3, spline.GetP4() };
