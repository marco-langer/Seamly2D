...
//---------------------------------------------------------------------------------------------------------------------
PatternPieceTool* PatternPieceTool::Create(
    ...
    VContainer* data)
{
    ...
    PatternPieceTool* patternPiece =
        Create(0, piece, width, scene, doc, data, Document::FullParse, Source::FromGui);
    ...
    const QString& blockName)
{
    ...
    if (parse == Document::FullParse) {
        ...
        connect(
            patternPiece, &PatternPieceTool::chosenTool, scene, &VMainGraphicsScene::chosenItem);
        connect(
            scene,
            &VMainGraphicsScene::EnableDetailItemHover,
            patternPiece,
            &PatternPieceTool::AllowHover);
        connect(
            scene,
            &VMainGraphicsScene::EnableDetailItemSelection,
            patternPiece,
            &PatternPieceTool::AllowSelecting);
        connect(
            scene, &VMainGraphicsScene::highlightPiece, patternPiece, &PatternPieceTool::Highlight);
        connect(
            scene,
            &VMainGraphicsScene::pieceLockedChanged,
            patternPiece,
            &PatternPieceTool::pieceLockedChanged);
...
//---------------------------------------------------------------------------------------------------------------------
void PatternPieceTool::insertNodes(
    ...
    VAbstractPattern* doc)
{
    ...
    if (pieceId > NULL_ID && !nodes.isEmpty()) {
        ...
        data->UpdatePiece(
            pieceId, newPiece);   // Update piece because first save will not call lite update
        connect(
...
//---------------------------------------------------------------------------------------------------------------------
void PatternPieceTool::AddAttributes(
    VAbstractPattern* doc, QDomElement& domElement, quint32 id, const VPiece& piece)
{
    SCASSERT(doc != nullptr);
    ...
    } else {   // For backward compatebility.
        domElement.removeAttribute(AttrSeamAllowanceBuiltIn);
    }

    doc->SetAttribute(
        domElement, VAbstractPattern::AttrWidth, piece.getSeamAllowanceWidthFormula());
    doc->SetAttribute(domElement, AttrUnited, piece.IsUnited());
}

//---------------------------------------------------------------------------------------------------------------------
void PatternPieceTool::AddCSARecord(
    VAbstractPattern* doc, QDomElement& domElement, const CustomSARecord& record)
{
    ...
    doc->SetAttribute(
        recordNode, VAbstractPattern::AttrIncludeAs, static_cast<unsigned int>(record.includeType));
...
//---------------------------------------------------------------------------------------------------------------------
void PatternPieceTool::AddPatternPieceData(
    VAbstractPattern* doc, QDomElement& domElement, const VPiece& piece)
{
...
//---------------------------------------------------------------------------------------------------------------------
void PatternPieceTool::AddPatternInfo(
    VAbstractPattern* doc, QDomElement& domElement, const VPiece& piece)
{
...
//---------------------------------------------------------------------------------------------------------------------
void PatternPieceTool::AddGrainline(
    VAbstractPattern* doc, QDomElement& domElement, const VPiece& piece)
{
    // grainline
...
//---------------------------------------------------------------------------------------------------------------------
QString PatternPieceTool::getTagName() const { return VAbstractPattern::TagPiece; }

//---------------------------------------------------------------------------------------------------------------------
void PatternPieceTool::ShowVisualization(bool show) { Q_UNUSED(show) }
...
void PatternPieceTool::pieceLockedChanged(quint32 id, bool lock)
{
    if (m_id == id) {
        ...
        for (int i = 0; i < piece.GetPath().CountNodes(); ++i) {
            ...
            if (node.GetTypeTool() == Tool::NodePoint) {
                VNodePoint* tool =
                    qobject_cast<VNodePoint*>(VAbstractPattern::getTool(node.GetId()));
                SCASSERT(tool != nullptr);
...
//---------------------------------------------------------------------------------------------------------------------
void PatternPieceTool::AllowHover(bool enabled) { setAcceptHoverEvents(enabled); }
...
void PatternPieceTool::UpdateGrainline()
{
    ...
    if (data.IsVisible() & qApp->Settings()->showGrainlines()) {
        ...
        m_grainLine->UpdateGeometry(
            pos,
            dRotation,
            ToPixel(dLength, *VDataTool::data.GetPatternUnit()),
            data.GetArrowType());
        m_grainLine->show();
    } else {
        m_grainLine->hide();
...
void PatternPieceTool::saveResizePiece(qreal dLabelW, int iFontSize)
{
    ...
    const qreal height =
        FromPixel(m_dataLabel->boundingRect().height(), *VDataTool::data.GetPatternUnit());
    ...
    connect(
        resizeCommand, &SavePieceOptions::NeedLiteParsing, doc, &VAbstractPattern::LiteParseTree);
...
void PatternPieceTool::savePieceRotation(qreal dRot)
{
    ...
    connect(
        rotateCommand, &SavePieceOptions::NeedLiteParsing, doc, &VAbstractPattern::LiteParseTree);
...
void PatternPieceTool::SaveResizePattern(qreal dLabelW, int iFontSize)
{
    ...
    qreal height =
        FromPixel(m_patternInfo->boundingRect().height(), *VDataTool::data.GetPatternUnit());
...
//---------------------------------------------------------------------------------------------------------------------
/**
 * @brief VToolDetail::paint draws a bounding box around piece, if one of its text or grainline
 * items is not idle.
 */
void PatternPieceTool::paint(
    QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
    ...
    m_cutLine->setPen(QPen(
        color,
        scaleWidth(lineWeight, sceneScale(scene())),
        lineTypeToPenStyle(lineType),
        Qt::RoundCap,
        Qt::RoundJoin));
    ...
    if ((m_dataLabel->IsIdle() == false || m_patternInfo->IsIdle() == false
         || m_grainLine->IsIdle() == false)
        && not isSelected()) {
        setSelected(true);
    }
...
void PatternPieceTool::RefreshDataInFile()
{
    ...
    if (domElement.isElement()) {
        // Refresh only parts that we possibly need to update
        {
            ...
            const uint version =
                doc->GetParametrUInt(domElement, PatternPieceTool::AttrVersion, "1");
            if (version == 1) {
                const VPiece& piece{ VAbstractTool::data.GetPiece(m_id) };
...
//---------------------------------------------------------------------------------------------------------------------
QVariant
PatternPieceTool::itemChange(QGraphicsItem::GraphicsItemChange change, const QVariant& value)
{
            ...
            if (not viewList.isEmpty()) {
                if (VMainGraphicsView* view = qobject_cast<VMainGraphicsView*>(viewList.at(0))) {
                    ...
                    const QRectF itemRect =
                        mapToScene(boundingRect() | childrenBoundingRect()).boundingRect();
                    ...
                    if (itemRect.height() + 2 * ymargin < viewRect.height()
                        ...
                        view->ensureVisible(QRectF(
                            cursorPosition.x() - 5 / scale,
...
void PatternPieceTool::contextMenuEvent(QGraphicsSceneContextMenuEvent* event)
{
    ...
    QAction* editProperties =
        menu.addAction(QIcon::fromTheme("preferences-other"), tr("Properties") + "\tP");
    ...
    QAction* showSeamAllowance = menu.addAction(
        QIcon("://icon/32x32/seam_allowance.png"), tr("Show Seam Allowance") + "\tS");
    showSeamAllowance->setCheckable(true);
    showSeamAllowance->setChecked(piece.IsSeamAllowance());

    QAction* showGrainline =
        menu.addAction(QIcon("://icon/32x32/grainline.png"), tr("Show Grainline") + "\tG");
    ...
    QAction* showPatternLabel =
        menu.addAction(QIcon("://icon/32x32/pattern_label.png"), tr("Show Pattern Label") + "\t[");
    showPatternLabel->setCheckable(true);
    showPatternLabel->setChecked(piece.GetPatternInfo().IsVisible());

    QAction* showPieceLabel =
        menu.addAction(QIcon("://icon/32x32/piece_label.png"), tr("Show Piece Label") + "\t]");
...
void PatternPieceTool::keyReleaseEvent(QKeyEvent* event)
{
    ...
    case Qt::Key_Bar: {
        if (piece.IsSeamAllowance() && qApp->Settings()->showSeamAllowances()) {
            toggleSeamLine(
                !piece.isHideSeamLine());   // Seam line is only valid if there is a seam allowance
        }
        break;
    }
...
//---------------------------------------------------------------------------------------------------------------------
PatternPieceTool::PatternPieceTool(
    VAbstractPattern* doc,
    VContainer* data,
    const quint32& id,
    const Source& typeCreation,
    ...
    , m_notches(new QGraphicsPathItem(this))
{
    ...
    connect(
        m_dataLabel, &VTextGraphicsItem::itemRotated, this, &PatternPieceTool::savePieceRotation);

    connect(m_patternInfo, &VTextGraphicsItem::itemMoved, this, &PatternPieceTool::SaveMovePattern);
    connect(
        m_patternInfo, &VTextGraphicsItem::itemResized, this, &PatternPieceTool::SaveResizePattern);
    connect(
        m_patternInfo,
        &VTextGraphicsItem::itemRotated,
        this,
        &PatternPieceTool::SaveRotationPattern);

    connect(m_grainLine, &VGrainlineItem::itemMoved, this, &PatternPieceTool::SaveMoveGrainline);
    connect(
        m_grainLine, &VGrainlineItem::itemResized, this, &PatternPieceTool::SaveResizeGrainline);
    connect(
        m_grainLine, &VGrainlineItem::itemRotated, this, &PatternPieceTool::SaveRotateGrainline);

    connect(
        doc, &VAbstractPattern::UpdatePatternLabel, this, &PatternPieceTool::UpdatePatternLabel);
    connect(doc, &VAbstractPattern::CheckLayout, this, &PatternPieceTool::updatePieceDetails);

    connect(
        m_pieceScene,
        &VMainGraphicsScene::DimensionsChanged,
        this,
        &PatternPieceTool::updatePieceDetails);
    connect(
        m_pieceScene, &VMainGraphicsScene::LanguageChanged, this, &PatternPieceTool::retranslateUi);
...
//---------------------------------------------------------------------------------------------------------------------
VPieceItem::MoveTypes PatternPieceTool::FindLabelGeometry(
    ...
    QPointF& pos)
{
    ...
    try {
        ...
        Calculator cal1;
        rotationAngle =
            cal1.EvalFormula(VAbstractTool::data.DataVariables(), labelData.GetRotation());
    ...
    try {
        ...
        Calculator cal1;
        labelWidth =
            cal1.EvalFormula(VAbstractTool::data.DataVariables(), labelData.GetLabelWidth());
        ...
        Calculator cal2;
        labelHeight =
            cal2.EvalFormula(VAbstractTool::data.DataVariables(), labelData.GetLabelHeight());
...
//---------------------------------------------------------------------------------------------------------------------
VPieceItem::MoveTypes PatternPieceTool::FindGrainlineGeometry(
    const VGrainlineData& data, qreal& length, qreal& rotationAngle, QPointF& pos)
{
...
//---------------------------------------------------------------------------------------------------------------------
void PatternPieceTool::initializeNode(
    ...
    PatternPieceTool* parent)
{
    ...
    case (Tool::NodePoint): {
        ...
        connect(
            tool,
            &VNodePoint::chosenTool,
            scene,
            &VMainGraphicsScene::chosenItem,
            Qt::UniqueConnection);
...
void PatternPieceTool::deleteTool(bool ask)
{
    ...
    // Because this object should be inactive from no one we disconnect all signals that may cause a
    // crash KEEP THIS LIST ACTUALL!!!
...
//---------------------------------------------------------------------------------------------------------------------
bool PatternPieceTool::PrepareLabelData(
    const VPatternLabelData& labelData,
    ...
    qreal& labelAngle)
{
    ...
    labelItem->setSize(
        ToPixel(labelWidth, *VDataTool::data.GetPatternUnit()),
...
{
    QSharedPointer<PatternPieceDialog> dialog = QSharedPointer<PatternPieceDialog>(
        new PatternPieceDialog(getData(), m_id, qApp->getMainWindow()));
    dialog->enableApply(true);
    m_dialog = dialog;
    m_dialog->setModal(true);
    connect(
        m_dialog.data(), &DialogTool::DialogClosed, this, &PatternPieceTool::FullUpdateFromGuiOk);
    connect(
        m_dialog.data(),
        &DialogTool::DialogApplied,
        this,
        &PatternPieceTool::FullUpdateFromGuiApply);
    SetDialog();
    m_dialog->show();
}
 ...
 */
void PatternPieceTool::renamePiece(const VPiece& piece)
{
    ...
    dialog->setWindowFlags(
        dialog->windowFlags() & ~Qt::WindowContextHelpButtonHint & ~Qt::WindowMaximizeButtonHint
    ...
    if (result == true && !pieceName.isEmpty()) {
        ...
        connect(
            undoCommand, &SavePieceOptions::NeedLiteParsing, doc, &VAbstractPattern::LiteParseTree);
        qApp->getUndoStack()->push(undoCommand);
