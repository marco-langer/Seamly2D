    ...
    , selectionType(SelectionType::ByMouseRelease)
{
    ...
    connect(
        this->doc, &VAbstractPattern::FullUpdateFromFile, this, &VAbstractTool::FullUpdateFromFile);
 ...
 *
 * Try calculate formula. If find error show dialog that allow user try fix formula. If user can't
 * throw exception. In successes case return result calculation and fixed formula string. If formula
 * ok don't touch formula.
 *
...
const QStringList VAbstractTool::Colors()
{
    const QStringList colors = QStringList()
                            << ColorBlack << ColorGreen << ColorBlue << ColorDarkRed
                            << ColorDarkGreen << ColorDarkBlue << ColorYellow << ColorLightSalmon
                            << ColorGoldenRod << ColorOrange << ColorDeepPink << ColorViolet
...
{
    const QStringList fills = QStringList()
                           << FillNone << FillSolid << FillDense1 << FillDense2 << FillDense3
                           << FillDense4 << FillDense5 << FillDense6 << FillDense7 << FillHorizLines
                           << FillVertLines << FillCross << FillBackwardDiagonal
                           << FillForwardDiagonal << FilldDiagonalCross;
    return fills;
}
...
//---------------------------------------------------------------------------------------------------------------------
void VAbstractTool::ToolSelectionType(const SelectionType& type) { selectionType = type; }
...
//---------------------------------------------------------------------------------------------------------------------
QDomElement
VAbstractTool::AddSANode(VAbstractPattern* doc, const QString& tagName, const VPieceNode& node)
{
    ...
    if (type != Tool::NodePoint) {
        doc->SetAttribute(
            nod, VAbstractPattern::AttrNodeReverse, static_cast<quint8>(node.GetReverse()));
    } else {
    ...
    if (type == Tool::NodePoint) {
        if (node.isNotch()) {
            ...
            doc->SetAttribute(
                nod, VAbstractPattern::AttrNodeShowSecondNotch, node.showSeamlineNotch());
            doc->SetAttribute(
                nod, VAbstractPattern::AttrNodeNotchType, notchTypeToString(node.getNotchType()));
            doc->SetAttribute(
                nod,
        ...
        if (not node.isNotch() && node.getNotchType() == NotchType::Slit
            && node.getNotchSubType()
                   == NotchSubType::Straightforward) {   // For backward compatebility.
...
//---------------------------------------------------------------------------------------------------------------------
QVector<VPieceNode> VAbstractTool::PrepareNodes(
    const VPiecePath& path, VMainGraphicsScene* scene, VAbstractPattern* doc, VContainer* data)
{
...
//---------------------------------------------------------------------------------------------------------------------
quint32 VAbstractTool::PrepareNode(
    const VPieceNode& node, VMainGraphicsScene* scene, VAbstractPattern* doc, VContainer* data)
{
