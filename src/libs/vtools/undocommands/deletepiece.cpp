...
DeletePiece::DeletePiece(
    VAbstractPattern* doc, quint32 id, const VPiece& piece, QUndoCommand* parent)
    ...
    , m_piece(piece)
{
    ...
    if (domElement.isElement()) {
        ...
        if (previousPiece.isNull()) {
            ...
            m_siblingId = doc->GetParametrUInt(
                previousPiece.toElement(), VAbstractPattern::AttrId, NULL_ID_STR);
        }
    } else {
...
//---------------------------------------------------------------------------------------------------------------------
DeletePiece::~DeletePiece() = default;
...
void DeletePiece::redo()
{
    ...
    if (domElement.isElement()) {
        ...
        // So when UnionDetail delete piece we can't use FullParsing. So we hide piece on scene
        // directly.
