...
dx_iface::dx_iface(
    const std::string& file, DRW::Version v, VarMeasurement varMeasurement, VarInsunits varInsunits)
...
{
    switch (e->eType) {
    case DRW::POINT: dxfW->writePoint(static_cast<DRW_Point*>(e)); break;
    case DRW::LINE: dxfW->writeLine(static_cast<DRW_Line*>(e)); break;
    case DRW::CIRCLE: dxfW->writeCircle(static_cast<DRW_Circle*>(e)); break;
    case DRW::ARC: dxfW->writeArc(static_cast<DRW_Arc*>(e)); break;
    case DRW::SOLID: dxfW->writeSolid(static_cast<DRW_Solid*>(e)); break;
    case DRW::ELLIPSE: dxfW->writeEllipse(static_cast<DRW_Ellipse*>(e)); break;
    case DRW::LWPOLYLINE: dxfW->writeLWPolyline(static_cast<DRW_LWPolyline*>(e)); break;
    case DRW::POLYLINE: dxfW->writePolyline(static_cast<DRW_Polyline*>(e)); break;
    case DRW::SPLINE:
        ...
        //        break;
    case DRW::INSERT: dxfW->writeInsert(static_cast<DRW_Insert*>(e)); break;
    case DRW::MTEXT: dxfW->writeMText(static_cast<DRW_MText*>(e)); break;
    case DRW::TEXT: dxfW->writeText(static_cast<DRW_Text*>(e)); break;
    case DRW::DIMLINEAR:
    case DRW::DIMALIGNED:
    case DRW::DIMANGULAR:
    ...
    case DRW::DIMORDINATE: dxfW->writeDimension(static_cast<DRW_Dimension*>(e)); break;
    case DRW::LEADER: dxfW->writeLeader(static_cast<DRW_Leader*>(e)); break;
    case DRW::HATCH: dxfW->writeHatch(static_cast<DRW_Hatch*>(e)); break;
    case DRW::IMAGE:
...
void dx_iface::writeHeader(DRW_Header& data)
{
    ...
    data = cData.headerC;
    // or copy one by one:
    //        for (std::map<std::string,DRW_Variant*>::iterator it=cData->headerC.vars.begin(); it
...
{
    // write each block
    for (std::list<dx_ifaceBlock*>::iterator it = cData.blocks.begin(); it != cData.blocks.end();
         ++it) {
        dx_ifaceBlock* bk = *it;
        dxfW->writeBlock(bk);
...
{
    for (std::list<DRW_LType>::iterator it = cData.lineTypes.begin(); it != cData.lineTypes.end();
         ++it)
...
void dx_iface::InitHeader(VarMeasurement varMeasurement, VarInsunits varInsunits)
{
    ...
    if (version >= DRW::AC1021) {   // Full support Unicode
...
void dx_iface::AddAAMAHeaderData() { cData.headerC.addStr("$CLAYER", "1", 8); }
...
UTF8STRING dx_iface::AddFont(const QFont& f)
{
    ...
    // Idea source
    // https://stackoverflow.com/questions/20111522/writing-text-styles-into-dxf-from-a-delphi-application
    if (f.bold()) {
        ts.name += "_BOLD";
...
void dx_iface::AddBlock(dx_ifaceBlock* block) { cData.blocks.push_back(block); }
