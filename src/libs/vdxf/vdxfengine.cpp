...
static inline QPaintEngine::PaintEngineFeatures svgEngineFeatures()
{
    ...
    return QPaintEngine::PaintEngineFeatures(
        QPaintEngine::AllFeatures & ~QPaintEngine::PatternBrush
...
bool VDxfEngine::begin(QPaintDevice* pdev)
{
    ...
    input = QSharedPointer<dx_iface>(
        new dx_iface(fileName.toStdString(), m_version, varMeasurement, varInsunits));
    input->AddQtLTypes();
    input->AddDefLayers();
    return true;
}
...
void VDxfEngine::drawPath(const QPainterPath& path)
{
    ...
    for (int j = 0; j < subpaths.size(); ++j) {
        ...
        if (m_version > DRW::AC1009) {   // Use lwpolyline
            ...
            for (int i = 0; i < polygon.count(); ++i) {
                poly->addVertex(DRW_Vertex2D(
                    FromPixel(polygon.at(i).x(), varInsunits),
...
void VDxfEngine::drawLines(const QLineF* lines, int lineCount)
{
    for (int i = 0; i < lineCount; ++i) {
        ...
        line->basePoint = DRW_Coord(
            FromPixel(p1.x(), varInsunits), FromPixel(getSize().height() - p1.y(), varInsunits), 0);
        line->secPoint = DRW_Coord(
            FromPixel(p2.x(), varInsunits), FromPixel(getSize().height() - p2.y(), varInsunits), 0);
        line->layer = "0";
...
//---------------------------------------------------------------------------------------------------------------------
void VDxfEngine::drawPolygon(
    const QPoint* points, int pointCount, QPaintEngine::PolygonDrawMode mode)
{
...
void VDxfEngine::drawEllipse(const QRectF& rect)
{
    ...
    if (rect.width() <= rect.height()) {
        majorX = (rect.top() - rect.center().y()) * sin(rotationAngle) * transform.m11()
               / cos(rotationAngle);
        // major axis * sin(rotation angle) * x-scale-factor
        majorY = (rect.top() - rect.center().y()) * transform.m22();
        // major axis * cos(rotation angle) * y-scale-factor, where y-scale-factor =
        // transform.m22()/cos(rotationAngle)
        ratio = rect.width() / rect.height();
    } else {
        majorX = (rect.right() - rect.center().x()) * transform.m11();
        // major axis * cos(rotation angle) * x-scale-factor, where y-scale-factor =
        // transform.m22()/cos(rotationAngle)
        majorY = (rect.right() - rect.center().x()) * sin(rotationAngle) * transform.m22()
               / cos(rotationAngle);
    ...
    ellipse->basePoint = DRW_Coord(
        FromPixel(newRect.center().x(), varInsunits),
        FromPixel(getSize().height() - newRect.center().y(), varInsunits),
        0);
    ellipse->secPoint =
        DRW_Coord(FromPixel(majorX, varInsunits), FromPixel(majorY, varInsunits), 0);
    ellipse->ratio = ratio;
    ellipse->staparam = 0;
...
//---------------------------------------------------------------------------------------------------------------------
void VDxfEngine::drawEllipse(const QRect& rect) { QPaintEngine::drawEllipse(rect); }
...
void VDxfEngine::drawTextItem(const QPointF& p, const QTextItem& textItem)
{
    ...
    /* Because QPaintEngine::drawTextItem doesn't pass whole string per time we mark end of each
     * string by adding special placholder. */
...
//---------------------------------------------------------------------------------------------------------------------
QPaintEngine::Type VDxfEngine::type() const { return QPaintEngine::User; }
...
//---------------------------------------------------------------------------------------------------------------------
double VDxfEngine::getResolution() const { return resolution; }
...
//---------------------------------------------------------------------------------------------------------------------
DRW::Version VDxfEngine::GetVersion() const { return m_version; }
...
//---------------------------------------------------------------------------------------------------------------------
void VDxfEngine::SetBinaryFormat(bool binary) { m_binary = binary; }

//---------------------------------------------------------------------------------------------------------------------
bool VDxfEngine::IsBinaryFormat() const { return m_binary; }
...
std::string VDxfEngine::getPenStyle()
{
    ...
    case Qt::DashDotLine: return "DASHDOT2";
    case Qt::DashDotDotLine: return "DIVIDE2";
    case Qt::SolidLine:
...
{
    switch (unit) {
    case VarInsunits::Millimeters: return pix / resolution * 25.4;
    case VarInsunits::Centimeters: return pix / resolution * 25.4 / 10.0;
    case VarInsunits::Inches: return pix / resolution;
    }
    return pix;
}
...
{
    switch (unit) {
    case VarInsunits::Millimeters: return (val / 25.4) * resolution;
    case VarInsunits::Centimeters: return ((val * 10.0) / 25.4) * resolution;
    case VarInsunits::Inches: return val * resolution;
    }
    return val;
}
...
void VDxfEngine::ExportAAMAText(dx_ifaceBlock* detailBlock, const VLayoutPiece& detail)
{
    ...
    for (int i = 0; i < list.size(); ++i) {
        QPointF pos(
            startPos.x(),
            startPos.y() - ToPixel(AAMATextHeight, varInsunits) * (list.size() - i - 1));
        detailBlock->ent.push_back(AAMAText(pos, list.at(i), "1"));
    }
}

//---------------------------------------------------------------------------------------------------------------------
void VDxfEngine::ExportAAMAGlobalText(
        ...
        if (not strings.isEmpty()) {
            for (int j = 0; j < strings.size(); ++j) {
                QPointF pos(
                    0,
                    getSize().height()
                        - ToPixel(AAMATextHeight, varInsunits) * (strings.size() - j - 1));
...
//---------------------------------------------------------------------------------------------------------------------
DRW_Entity*
VDxfEngine::AAMAPolygon(const QVector<QPointF>& polygon, const QString& layer, bool forceClosed)
{
    if (polygon.isEmpty()) {
        return nullptr;
    }
...
DRW_Entity* VDxfEngine::AAMALine(const QLineF& line, const QString& layer)
{
    ...
    lineEnt->basePoint = DRW_Coord(
        FromPixel(line.p1().x(), varInsunits),
        FromPixel(getSize().height() - line.p1().y(), varInsunits),
        0);
    lineEnt->secPoint = DRW_Coord(
        FromPixel(line.p2().x(), varInsunits),
        FromPixel(getSize().height() - line.p2().y(), varInsunits),
        0);
...
DRW_Entity* VDxfEngine::AAMAText(const QPointF& pos, const QString& text, const QString& layer)
{
    ...
    textLine->basePoint = DRW_Coord(
        FromPixel(pos.x(), varInsunits), FromPixel(getSize().height() - pos.y(), varInsunits), 0);
    textLine->secPoint = DRW_Coord(
        FromPixel(pos.x(), varInsunits), FromPixel(getSize().height() - pos.y(), varInsunits), 0);
...
P* VDxfEngine::CreateAAMAPolygon(
    const QVector<QPointF>& polygon, const QString& layer, bool forceClosed)
{
    P* poly = new P();
    ...
    for (int i = 0; i < polygon.count(); ++i) {
        poly->addVertex(
            V(FromPixel(polygon.at(i).x(), varInsunits),
              FromPixel(getSize().height() - polygon.at(i).y(), varInsunits)));
    }
