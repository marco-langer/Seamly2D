...
public:
        ...
        , fitpoint()
    {
        ...
        for (double v : p.knotslist)
            knotslist.push_back(v);
        for (double v : p.weightlist)
            weightlist.push_back(v);
        for (DRW_Coord* v : p.controllist)
            controllist.push_back(new DRW_Coord(*v));
        for (DRW_Coord* v : p.fitlist)
            fitlist.push_back(new DRW_Coord(*v));
    }
...
private:
    ...
    double linefactor;   /*!< Dimension text line spacing factor, code 41, default 1? (value range
                            0.25 to 4.00*/
    ...
    DRW_Coord
        clonePoint; /*!< Insertion point for clones (Baseline & Continue), code 12, 22 & 32 (OCS) */
    ...
    DRW_Coord circlePoint; /*!< Definition point for diameter, radius & angular dims code 15, 25 &
                              35 (WCS) */
...
};   // RLZ: missing 15,25, 72, 331, 90, 340, 1, 281, 71, 74, 110, 120, 130, 111, 121,131, 112,122,
     // 132, 345,346, and more...
