...
void VAbstractPiece::Swap(VAbstractPiece& piece) Q_DECL_NOTHROW { std::swap(d, piece.d); }
...
//---------------------------------------------------------------------------------------------------------------------
bool VAbstractPiece::getLock() const { return d->m_pieceLock; }

//---------------------------------------------------------------------------------------------------------------------
void VAbstractPiece::setLock(bool value) { d->m_pieceLock = value; }

//---------------------------------------------------------------------------------------------------------------------
bool VAbstractPiece::IsForbidFlipping() const { return d->m_forbidFlipping; }

//---------------------------------------------------------------------------------------------------------------------
void VAbstractPiece::SetForbidFlipping(bool value) { d->m_forbidFlipping = value; }

//---------------------------------------------------------------------------------------------------------------------
bool VAbstractPiece::IsSeamAllowance() const { return d->m_seamAllowance; }

//---------------------------------------------------------------------------------------------------------------------
void VAbstractPiece::SetSeamAllowance(bool value) { d->m_seamAllowance = value; }

//---------------------------------------------------------------------------------------------------------------------
bool VAbstractPiece::IsSeamAllowanceBuiltIn() const { return d->m_seamAllowanceBuiltIn; }

//---------------------------------------------------------------------------------------------------------------------
void VAbstractPiece::SetSeamAllowanceBuiltIn(bool value) { d->m_seamAllowanceBuiltIn = value; }

//---------------------------------------------------------------------------------------------------------------------
bool VAbstractPiece::isHideSeamLine() const { return d->m_hideMainPath; }

//---------------------------------------------------------------------------------------------------------------------
void VAbstractPiece::setHideSeamLine(bool value) { d->m_hideMainPath = value; }

//---------------------------------------------------------------------------------------------------------------------
qreal VAbstractPiece::GetSAWidth() const { return d->m_width; }

//---------------------------------------------------------------------------------------------------------------------
void VAbstractPiece::SetSAWidth(qreal value) { value >= 0 ? d->m_width = value : d->m_width = 0; }
...
QVector<QPointF> VAbstractPiece::Equidistant(const QVector<VSAPoint>& points, qreal width)
{
    ...
    for (qint32 i = 0; i < p.size(); ++i) {
        if (i == 0) {   // first point
            ekvPoints << EkvPoint(p.at(p.size() - 2), p.at(p.size() - 1), p.at(1), p.at(0), width);
            continue;
        }

        if (i == p.size() - 1) {   // last point
        ...
        ekvPoints << EkvPoint(p.at(i - 1), p.at(i), p.at(i + 1), p.at(i), width);
    }

    const bool removeFirstAndLast = false;
    ekvPoints = CheckLoops(
        CorrectEquidistantPoints(ekvPoints, removeFirstAndLast));   // Result path can contain loops
    return ekvPoints;
}
...
QVector<QPointF> VAbstractPiece::CheckLoops(const QVector<QPointF>& points)
{
    ...
    for (i = 0; i < count; ++i) {
        ...
        // Because a path can contains several loops we will seek the last and only then remove the
        // loop(s) That's why we parse from the end
        for (j = count - 1; j >= i + 2; --j) {
            j == count - 1 ? jNext = 0 : jNext = j + 1;
            QLineF line2(points.at(j), points.at(jNext));

            if (qFuzzyIsNull(line2.length())) {   // If a path is closed the edge (count-1;0) length
                                                  // will be 0
                continue;
            }
            ...
            if (intersect
                == QLineF::NoIntersection) {   // According to the documentation
                                               // QLineF::NoIntersection indicates that the lines do
                                               // not intersect; i.e. they are parallel. But
                                               // parallel also mean they can be on the same line.
                ...
                if (VGObject::IsPointOnLineviaPDP(points.at(j), points.at(i), points.at(i + 1))
                    ...
                    // Left to catch case where segments are on the same line, but do not have real
                    // intersections.
                    ...
                    const QLineF::IntersectType tmpIntrs1 =
                        tmpLine1.intersects(tmpLine2, &tmpCrosPoint);

                    tmpLine1 = line1;
                    tmpLine2.setAngle(tmpLine2.angle() + 90);

                    const QLineF::IntersectType tmpIntrs2 =
                        tmpLine1.intersects(tmpLine2, &tmpCrosPoint);

                    if (tmpIntrs1 == QLineF::BoundedIntersection
                        || tmpIntrs2
                               == QLineF::BoundedIntersection) {   // Now we really sure that lines
                                                                   // are on the same lines and have
                                                                   // real intersections.
                        QPointF cPoint;
                ...
                if (uniqueVertices.size() == 4) {   // Break, but not if lines are neighbors
...
//---------------------------------------------------------------------------------------------------------------------
/**
 * @brief EkvPoint return seam allowance points in place of intersection of two edges. Last points
 * of two edges should be equal.
 * @param width global seam allowance width.
 * @return seam allowance points.
 */
QVector<QPointF> VAbstractPiece::EkvPoint(
    ...
    case (QLineF::UnboundedIntersection): {   // Most common case
        ...
        {                                                            // Regular equdistant case
            ...
            case PieceNodeAngle::ByPointsIntersection:
                return AngleByIntersection(
                    p1Line1, p2Line1, p1Line2, bigLine1.p1(), CrosPoint, bigLine2.p2(), localWidth);
            case PieceNodeAngle::ByFirstEdgeSymmetry:
                return AngleByFirstSymmetry(
                    p1Line1, p2Line1, bigLine1.p1(), CrosPoint, bigLine2.p2(), localWidth);
            case PieceNodeAngle::BySecondEdgeSymmetry:
                return AngleBySecondSymmetry(
                    p2Line1, p1Line2, bigLine1.p1(), CrosPoint, bigLine2.p2(), localWidth);
            case PieceNodeAngle::ByFirstEdgeRightAngle:
                return AngleByFirstRightAngle(
                    p1Line1, p2Line1, bigLine1.p1(), CrosPoint, bigLine2.p2(), localWidth);
            ...
            if ((result1 < 0 || qFuzzyIsNull(result1))
                && (result2 < 0
                    || qFuzzyIsNull(
                        result2))) {   // Dart case. A bisector watch outside. In some cases a point
                                       // still valid, but ignore if going outside of an equdistant.
            ...
            } else {   // New subcase. This is not a dart. An angle is acute and bisector watch
                       // inside.
                ...
                if ((result1 < 0 || qFuzzyIsNull(result1))
                    && (result2 < 0
                        || qFuzzyIsNull(
                            result2))) {   // The cross point is still outside of a piece
                    if (line.length() >= localWidth) {
                        points.append(CrosPoint);
                        return points;
                    } else {   // but not enough far, fix it
                        line.setLength(localWidth);
                        points.append(line.p2());
                        return points;
                    }
                } else {   // Wrong cross point, probably inside of a piece. Manually creating
                           // correct seam allowance
                    const QLineF bigEdge =
                        createParallelLine(bigLine1.p2(), bigLine2.p1(), localWidth);
...
//---------------------------------------------------------------------------------------------------------------------
QVector<QPointF> VAbstractPiece::AngleByLength(
    const QPointF& p2, const QPointF& sp1, const QPointF& sp2, const QPointF& sp3, qreal width)
{
    ...
    if (length > width * maxL) {   // Cutting too long a cut angle
...
//---------------------------------------------------------------------------------------------------------------------
QVector<QPointF> VAbstractPiece::AngleByIntersection(
    const QPointF& p1,
    const QPointF& p2,
    const QPointF& p3,
    const QPointF& sp1,
...
//---------------------------------------------------------------------------------------------------------------------
QVector<QPointF> VAbstractPiece::AngleByFirstSymmetry(
    const QPointF& p1,
    const QPointF& p2,
    const QPointF& sp1,
    const QPointF& sp2,
...
//---------------------------------------------------------------------------------------------------------------------
QVector<QPointF> VAbstractPiece::AngleBySecondSymmetry(
    const QPointF& p2,
    const QPointF& p3,
    const QPointF& sp1,
    const QPointF& sp2,
...
//---------------------------------------------------------------------------------------------------------------------
QVector<QPointF> VAbstractPiece::AngleByFirstRightAngle(
    const QPointF& p1,
    const QPointF& p2,
    const QPointF& sp1,
    const QPointF& sp2,
...
//---------------------------------------------------------------------------------------------------------------------
QVector<QPointF> VAbstractPiece::AngleBySecondRightAngle(
    const QPointF& p2,
    const QPointF& p3,
    const QPointF& sp1,
    const QPointF& sp2,
...
QLineF VAbstractPiece::createParallelLine(const VSAPoint& p1, const VSAPoint& p2, qreal width)
{
    ...
    const QLineF parallel =
        QLineF(SingleParallelPoint(p1, p2, 90, w1), SingleParallelPoint(p2, p1, -90, w2));
    return parallel;
}
...
{
    const QLineF parallel =
        QLineF(SingleParallelPoint(p1, p2, 90, width), SingleParallelPoint(p2, p1, -90, width));
    return parallel;
}

//---------------------------------------------------------------------------------------------------------------------
QPointF
VAbstractPiece::SingleParallelPoint(const QPointF& p1, const QPointF& p2, qreal angle, qreal width)
{
    QLineF pLine(p1, p2);
...
//---------------------------------------------------------------------------------------------------------------------
bool VAbstractPiece::IsEkvPointOnLine(
    const QPointF& iPoint, const QPointF& prevPoint, const QPointF& nextPoint)
{
...
//---------------------------------------------------------------------------------------------------------------------
bool VAbstractPiece::IsEkvPointOnLine(
    const VSAPoint& iPoint, const VSAPoint& prevPoint, const VSAPoint& nextPoint)
{
    // See bug #671
    ...
    return (
        VGObject::IsPointOnLineviaPDP(iPoint, prevPoint, nextPoint)
        ...
        && qAbs(prevPoint.GetSAAfter(tmpWidth) - nextPoint.GetSABefore(tmpWidth))
               < VGObject::accuracyPointOnLine);
}

//---------------------------------------------------------------------------------------------------------------------
qreal VAbstractPiece::GetMx() const { return d->m_mx; }

//---------------------------------------------------------------------------------------------------------------------
void VAbstractPiece::SetMx(qreal value) { d->m_mx = value; }

//---------------------------------------------------------------------------------------------------------------------
qreal VAbstractPiece::GetMy() const { return d->m_my; }

//---------------------------------------------------------------------------------------------------------------------
void VAbstractPiece::SetMy(qreal value) { d->m_my = value; }
