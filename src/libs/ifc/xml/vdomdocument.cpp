 ...
 */
quint32 VDomDocument::GetParametrUInt(
    const QDomElement& domElement, const QString& name, const QString& defValue)
{
...
//---------------------------------------------------------------------------------------------------------------------
bool VDomDocument::getParameterBool(
    const QDomElement& domElement, const QString& name, const QString& defValue)
{
    Q_ASSERT_X(not name.isEmpty(), Q_FUNC_INFO, "name of parametr is empty");
    ...
    try {
        ...
        const QStringList bools = QStringList() << QLatin1String("true") << QLatin1String("false")
...
//---------------------------------------------------------------------------------------------------------------------
void VDomDocument::SetParametrUsage(
    QDomElement& domElement, const QString& name, const NodeUsage& value)
{
 ...
 */
QString VDomDocument::GetParametrString(
    const QDomElement& domElement, const QString& name, const QString& defValue)
{
    Q_ASSERT_X(not name.isEmpty(), Q_FUNC_INFO, "name of parametr is empty");
    Q_ASSERT_X(not domElement.isNull(), Q_FUNC_INFO, "domElement is null");
 ...
 * @return double value
 */
qreal VDomDocument::GetParametrDouble(
    const QDomElement& domElement, const QString& name, const QString& defValue)
{
    Q_ASSERT_X(not name.isEmpty(), Q_FUNC_INFO, "name of parametr is empty");
    Q_ASSERT_X(not domElement.isNull(), Q_FUNC_INFO, "domElement is null");
...
void VDomDocument::setXMLContent(const QString& fileName)
{
    ...
    if (QDomDocument::setContent(&file, &errorMsg, &errorLine, &errorColumn) == false) {
        ...
        e.AddMoreInformation(tr("Parsing error file %3 in line %1 column %2")
                                 .arg(errorLine)
                                 .arg(errorColumn)
                                 .arg(fileName));
        throw e;
...
{
    QString r;
    for (auto i = static_cast<int>(Unit::Mm), last = static_cast<int>(Unit::LAST_UNIT_DO_NOT_USE);
         i < last;
         ++i) {
    ...
    if (not element.isNull()) {
        ...
        while (tagLine.isNull() == false) {
            if (tagLine.tagName() == TagLine) {
                ...
                line.fontSizeIncrement =
                    static_cast<int>(GetParametrUInt(tagLine, AttrFSIncrement, "0"));
                lines.append(line);
            }
