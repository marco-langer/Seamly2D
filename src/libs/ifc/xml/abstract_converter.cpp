...
//---------------------------------------------------------------------------------------------------------------------
int VAbstractConverter::GetCurrentFormatVarsion() const { return m_ver; }
...
void VAbstractConverter::ReserveFile() const
{
    ...
    const QString baseFileName =
        QString("%1_v%2").arg(baseNameWithoutVersion).arg(versionWithoutDots);
...
void VAbstractConverter::Replace(
    QString& formula, const QString& newName, int position, const QString& token, int& bias) const
{
...
//---------------------------------------------------------------------------------------------------------------------
void VAbstractConverter::CorrectionsPositions(
    int position, int bias, QMap<int, QString>& tokens) const
{
    if (bias == 0) {
...
[[noreturn]] void VAbstractConverter::InvalidVersion(int ver) const
{
    if (ver < minVer()) {
        const QString errorMsg(
            tr("Invalid version. Minimum supported version is %1").arg(minVerStr()));
        throw VException(errorMsg);
    }

    if (ver > maxVer()) {
        const QString errorMsg(
            tr("Invalid version. Maximum supported version is %1").arg(maxVerStr()));
        throw VException(errorMsg);
    }
...
void VAbstractConverter::ValidateInputFile(const QString& currentSchema) const
{
    ...
    try {
        ...
        if (m_ver
            < minVer()) {   // Version less than minimally supported version. Can't do anything.
            throw;
        ...
        } else {   // Unexpected version. Most time mean that we do not catch all versions between
                   // min and max.
            throw;
        }
