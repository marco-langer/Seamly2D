...
namespace {
void ReadExpressionAttribute(
    QVector<VFormulaField>& expressions, const QDomElement& element, const QString& attribute)
{
...
QStringList VAbstractPattern::ListMeasurements() const
{
    ...
    for (int i = 0; i < expressions.size(); ++i) {
        ...
        for (int j = 0; j < tValues.size(); ++j) {
            ...
            if (IsVariable(tValues.at(j)) || IsPostfixOperator(tValues.at(j))
                || IsFunction(tValues.at(j))) {
 ...
 */
QString VAbstractPattern::getActiveDraftBlockName() const { return activeDraftBlock; }
        ...
        if (domNode.isElement()) {
            ...
            if (domElement.isNull() == false) {
                if (domElement.tagName() == TagGroup) {
                    ...
                    const QPair<bool, QMap<quint32, quint32>> groupData =
...
bool VAbstractPattern::renameDraftBlock(const QString& oldName, const QString& newName)
{
    ...
    if (ppElement.isElement()) {
        ...
        emit patternChanged(
            false);   // For situation when we change name directly, without undocommands.
 ...
 *
 * Method check if not exist draft block with the same name and change name active draft block name,
 * send signal about change draft block. Doen't add draft block to file structure. This task make
 * SPoint tool.
 * @param name draft block name.
 * @return true if success.
 */
...
//---------------------------------------------------------------------------------------------------------------------
quint32 VAbstractPattern::getCursor() const { return cursor; }
...
//---------------------------------------------------------------------------------------------------------------------
void VAbstractPattern::setDefaultBasePoint(QString basePoint) { defaultBasePoint = basePoint; }

//---------------------------------------------------------------------------------------------------------------------
QString VAbstractPattern::getDefaultLineColor() const { return m_DefaultLineColor; }

//---------------------------------------------------------------------------------------------------------------------
qreal VAbstractPattern::getDefaultLineWeight() const { return m_DefaultLineWeight; }

//---------------------------------------------------------------------------------------------------------------------
QString VAbstractPattern::getDefaultLineType() const { return m_DefaultLineType; }
...
//---------------------------------------------------------------------------------------------------------------------
void VAbstractPattern::RemoveTool(quint32 id) { tools.remove(id); }
    ...
    for (qint32 i = 0; i < nodeList.size(); ++i) {
        ...
        if (not element.isNull()) {
            ...
            record.includeType = static_cast<PiecePathIncludeType>(
                GetParametrUInt(element, VAbstractPattern::AttrIncludeAs, "1"));
...
VPieceNode VAbstractPattern::ParseSANode(const QDomElement& domElement)
{
    ...
    const bool reverse =
        VDomDocument::GetParametrUInt(domElement, VAbstractPattern::AttrNodeReverse, "0");
    const bool excluded =
        VDomDocument::getParameterBool(domElement, VAbstractPattern::AttrNodeExcluded, falseStr);
    const QString saBefore = VDomDocument::GetParametrString(
        domElement, VAbstractPattern::AttrSABefore, currentSeamAllowance);
    const QString saAfter = VDomDocument::GetParametrString(
        domElement, VAbstractPattern::AttrSAAfter, currentSeamAllowance);
    const PieceNodeAngle angle =
        static_cast<PieceNodeAngle>(VDomDocument::GetParametrUInt(domElement, AttrAngle, "0"));

    const bool notch =
        VDomDocument::getParameterBool(domElement, VAbstractPattern::AttrNodeIsNotch, falseStr);
    const NotchType notchType = stringToNotchType(
        VDomDocument::GetParametrString(domElement, VAbstractPattern::AttrNodeNotchType, strSlit));
    const NotchSubType notchSubType = stringToNotchSubType(VDomDocument::GetParametrString(
        domElement, VAbstractPattern::AttrNodeNotchSubType, strStraightforward));

    const bool showNotch =
        VDomDocument::getParameterBool(domElement, VAbstractPattern::AttrNodeShowNotch, trueStr);
    const bool showSecond = VDomDocument::getParameterBool(
        domElement, VAbstractPattern::AttrNodeShowSecondNotch, trueStr);
    const qreal notchLength = VDomDocument::GetParametrDouble(
        domElement,
        VAbstractPattern::AttrNodeNotchLength,
        QString::number(qApp->Settings()->getDefaultNotchLength()));
    const qreal notchWidth = VDomDocument::GetParametrDouble(
        domElement,
        VAbstractPattern::AttrNodeNotchWidth,
        QString::number(qApp->Settings()->getDefaultNotchWidth()));
    const qreal notchAngle =
        VDomDocument::GetParametrDouble(domElement, VAbstractPattern::AttrNodeNotchAngle, ".00");
    const quint32 notchCount =
        VDomDocument::GetParametrUInt(domElement, VAbstractPattern::AttrNodeNotchCount, "1");


    const QString t =
        VDomDocument::GetParametrString(domElement, AttrType, VAbstractPattern::NodePoint);
    Tool tool;
    ...
    case 4:   // NodeElArc
        tool = Tool::NodeElArc;
        break;
    default: VException e(QObject::tr("Wrong tag name '%1'.").arg(t)); throw e;
    }
...
//---------------------------------------------------------------------------------------------------------------------
QString VAbstractPattern::MPath() const { return UniqueTagText(TagMeasurements); }
...
//---------------------------------------------------------------------------------------------------------------------
QStringList VAbstractPattern::getPatternPieces() const { return patternPieces; }
...
//---------------------------------------------------------------------------------------------------------------------
QString VAbstractPattern::GetDescription() const { return UniqueTagText(TagDescription); }
...
//---------------------------------------------------------------------------------------------------------------------
QString VAbstractPattern::GetNotes() const { return UniqueTagText(TagNotes); }
...
//---------------------------------------------------------------------------------------------------------------------
QString VAbstractPattern::GetPatternName() const { return UniqueTagText(TagPatternName); }
...
//---------------------------------------------------------------------------------------------------------------------
QString VAbstractPattern::GetCompanyName() const { return UniqueTagText(TagCompanyName); }
...
//---------------------------------------------------------------------------------------------------------------------
QString VAbstractPattern::GetPatternNumber() const { return UniqueTagText(TagPatternNum); }
...
//---------------------------------------------------------------------------------------------------------------------
QString VAbstractPattern::GetCustomerName() const { return UniqueTagText(TagCustomerName); }
...
//---------------------------------------------------------------------------------------------------------------------
void VAbstractPattern::SetPatternWasChanged(bool changed) { patternLabelWasChanged = changed; }

//---------------------------------------------------------------------------------------------------------------------
bool VAbstractPattern::GetPatternWasChanged() const { return patternLabelWasChanged; }

//---------------------------------------------------------------------------------------------------------------------
QString VAbstractPattern::GetImage() const { return UniqueTagText(TagImage); }
 ...
 */
void VAbstractPattern::haveLiteChange() { emit patternChanged(false); }

//---------------------------------------------------------------------------------------------------------------------
void VAbstractPattern::NeedFullParsing() { emit UndoCommand(); }

//---------------------------------------------------------------------------------------------------------------------
void VAbstractPattern::ClearScene() { emit ClearMainWindow(); }

//---------------------------------------------------------------------------------------------------------------------
void VAbstractPattern::updatePieceList(quint32 id) { emit UpdateInLayoutList(id); }

//---------------------------------------------------------------------------------------------------------------------
void VAbstractPattern::selectedPiece(quint32 id) { emit showPiece(id); }
...
QDomElement VAbstractPattern::CheckTagExists(const QString& tag)
{
    ...
    if (list.isEmpty()) {
        const QStringList tags = QStringList()
                              << TagUnit << TagImage << TagDescription << TagNotes << TagGradation
                              << TagPatternName << TagPatternNum << TagCompanyName
...
QVector<VFormulaField> VAbstractPattern::ListExpressions() const
{
    ...
    // Note. Tool Union Details also contains formulas, but we don't use them for union and keep
    // only to simplifying working with nodes. Same code for saving reading.
...
bool VAbstractPattern::IsVariable(const QString& token) const
{
    for (int i = 0; i < builInVariables.size(); ++i) {
        if (token.indexOf(builInVariables.at(i)) == 0) {
            if (builInVariables.at(i) == currentLength
                || builInVariables.at(i) == currentSeamAllowance) {
...
//---------------------------------------------------------------------------------------------------------------------
QPair<bool, QMap<quint32, quint32>>
VAbstractPattern::parseItemElement(const QDomElement& domElement)
{
    Q_ASSERT_X(not domElement.isNull(), Q_FUNC_INFO, "domElement is null");
 ...
 */
bool VAbstractPattern::IsModified() const { return modified; }

//---------------------------------------------------------------------------------------------------------------------
void VAbstractPattern::SetModified(bool modified) { this->modified = modified; }
...
QDomElement
VAbstractPattern::addGroupItems(const QString& name, const QMap<quint32, quint32>& groupData)
{
                ...
                if (domNode.isElement()) {
                    ...
                    if (group.isNull() == false) {
                        if (group.tagName() == TagGroup) {
                            ...
                            const QString name =
                                GetParametrString(group, AttrName, tr("New group 2"));
                            ...
                            const QString color =
                                GetParametrString(group, AttrGroupColor, ColorBlack);
                            const QString linetype =
                                GetParametrString(group, AttrLineType, LineTypeSolidLine);
                            const QString lineweight =
                                GetParametrString(group, AttrLineWeight, "0.35");
...
//---------------------------------------------------------------------------------------------------------------------
/**
 * @brief Returns the groups that contain or do not contain the item identified by the toolid and
 * the objectid
 * @param toolId
 * @param objectId
 * @param containsItem |true if the groups contain the given item, false if they don't contain the
 * item
 * @return
 */
QMap<quint32, QString>
VAbstractPattern::getGroupsContainingItem(quint32 toolId, quint32 objectId, bool containsItem)
{
            ...
            if (domNode.isElement()) {
                ...
                if (group.isNull() == false) {
                    if (group.tagName() == TagGroup) {
                        ...
                        if ((containsItem && groupHasItem)
                            || (not containsItem && not groupHasItem)) {
                            const quint32 groupId = GetParametrUInt(group, AttrId, "0");
                            const QString name =
                                GetParametrString(group, AttrName, tr("New group"));
 ...
 * @return
 */
bool VAbstractPattern::hasGroupItem(
    const QDomElement& groupDomElement, quint32 toolId, quint32 objectId)
{
    bool result = false;
...
void VAbstractPattern::addToolToGroup(quint32 toolId, quint32 objectId, const QString& groupName)
{
    ...
    // Add to new Group.
    QMap<quint32, QString> groupsNotContainingItem =
        getGroupsContainingItem(toolId, objectId, false);
            ...
            if (itemNode.isElement()) {
                ...
                if (item.isNull() == false) {
                    ...
                    if (toolIdIterate == toolId && objectIdIterate == objectId) {
                        ...
                        // parse the groups to update the drawing, in case the item was removed from
                        // an invisible group
                        QDomElement groups = createGroups();
                        if (not groups.isNull()) {
                            parseGroups(groups);
...
QString VAbstractPattern::useGroupColor(quint32 toolId, QString color)
{
    ...
    // qCDebug(vbstractPattern,"VAbstractPattern::useGroupColor - Group Name =
    // %s",qUtf8Printable(groupName)); qCDebug(vbstractPattern,"VAbstractPattern::useGroupColor -
    // Tool ID = %d",toolId); qCDebug(vbstractPattern,"VAbstractPattern::useGroupColor - Tool Color
    // = %s",qUtf8Printable(color));
    quint32 objectId = toolId;
    bool groupHasItem = hasGroupItem(getGroupByName(groupName), toolId, objectId);
    // qCDebug(vbstractPattern,"VAbstractPattern::useGroupColor - Group has item =
    // %d",groupHasItem);
    if ((color == ColorByGroup) && groupHasItem) {
        QString groupColor = getGroupColor(getGroupIdByName(groupName));
        // qCDebug(vbstractPattern,"VAbstractPattern::useGroupColor - Group Color =
        // %s",qUtf8Printable(groupColor));
        return groupColor;
    } else {
        return color;
    }
...
QString VAbstractPattern::useGroupLineType(quint32 toolId, QString type)
{
    ...
    // qCDebug(vbstractPattern,"VAbstractPattern::useGroupLineType - Group Name =
    // %s",qUtf8Printable(groupName)); qCDebug(vbstractPattern,"VAbstractPattern::useGroupLineType -
    // Tool ID = %d",toolId); qCDebug(vbstractPattern,"VAbstractPattern::useGroupLineType - Tool
    // Color = %s",qUtf8Printable(type));
    quint32 objectId = toolId;
    bool groupHasItem = hasGroupItem(getGroupByName(groupName), toolId, objectId);
    // qCDebug(vbstractPattern,"VAbstractPattern::useGroupLineType - Group has item =
    // %d",groupHasItem);
    if ((type == LineTypeByGroup) && groupHasItem) {
        QString groupLineType = getGroupLineType(getGroupIdByName(groupName));
        // qCDebug(vbstractPattern,"VAbstractPattern::useGroupLineType - Group Linetpe =
        // %s",qUtf8Printable(groupLineType));
...
QMap<qint32, ImageItem*> VAbstractPattern::getBackgroundImageMap() { return m_imageMap; }
...
void VAbstractPattern::removeBackgroundImage(qint32 id) { m_imageMap.remove(id); }


ImageItem* VAbstractPattern::getBackgroundImage(qint32 id) { return m_imageMap.value(id); }

void VAbstractPattern::clearBackgroundImageMap() { m_imageMap.clear(); }
