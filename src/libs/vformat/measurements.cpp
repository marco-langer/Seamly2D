...
QString FileComment()
{
    return QString("Measurements created with SeamlyMe v%1 (https://seamly.io/).")
        .arg(APP_VERSION_STR);
}
}   // namespace
...
MeasurementDoc::MeasurementDoc(VContainer* data)
    ...
    , m_currentHeight(nullptr){ SCASSERT(data != nullptr) }
...
//---------------------------------------------------------------------------------------------------------------------
void MeasurementDoc::AddEmptyAfter(
    const QString& after, const QString& name, const QString& formula)
{
    const QDomElement element = MakeEmpty(name, formula);
    ...
    for (int i = 0; i < list.size(); ++i) {
        ...
        if (type == MeasurementsType::Multisize) {
            ...
            const qreal baseSize =
                UnitConvertor(BaseSize(), measurementUnits(), *data->GetPatternUnit());
...
//---------------------------------------------------------------------------------------------------------------------
MeasurementsType MeasurementDoc::Type() const { return type; }
...
//---------------------------------------------------------------------------------------------------------------------
QString MeasurementDoc::Notes() const { return UniqueTagText(TagNotes, ""); }
...
//---------------------------------------------------------------------------------------------------------------------
QString MeasurementDoc::FamilyName() const { return UniqueTagText(TagFamilyName, ""); }
...
//---------------------------------------------------------------------------------------------------------------------
QString MeasurementDoc::GivenName() const { return UniqueTagText(TagGivenName, ""); }
...
//---------------------------------------------------------------------------------------------------------------------
QString MeasurementDoc::PMSystem() const { return UniqueTagText(TagPMSystem, ClearPMCode(p998_S)); }
...
//---------------------------------------------------------------------------------------------------------------------
QString MeasurementDoc::Email() const { return UniqueTagText(TagEmail, ""); }
...
//---------------------------------------------------------------------------------------------------------------------
bool MeasurementDoc::isReadOnly() const { return UniqueTagText(TagReadOnly, falseStr) == trueStr; }
...
//---------------------------------------------------------------------------------------------------------------------
void MeasurementDoc::setSize(qreal* size) { m_currentSize = size; }

//---------------------------------------------------------------------------------------------------------------------
void MeasurementDoc::setHeight(qreal* height) { m_currentHeight = height; }
...
//---------------------------------------------------------------------------------------------------------------------
VContainer* MeasurementDoc::GetData() const { return data; }
