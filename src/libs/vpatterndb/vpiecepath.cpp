...
void VPiecePath::Swap(VPiecePath& path) Q_DECL_NOTHROW { std::swap(d, path.d); }
...
QVector<QPointF> VPiecePath::PathPoints(const VContainer* data) const
{
    ...
    for (int i = 0; i < CountNodes(); ++i) {
        ...
        case (Tool::NodeSplinePath): {
            const QSharedPointer<VAbstractCurve> curve =
                data->GeometricObject<VAbstractCurve>(at(i).GetId());
    ...
    const VContainer* data, const QVector<VPieceNode>& nodes, int i, bool reverse)
{
    ...
    const QSharedPointer<VAbstractCurve> curve =
        data->GeometricObject<VAbstractCurve>(nodes.at(i).GetId());
