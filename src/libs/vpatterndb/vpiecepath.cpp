...
void VPiecePath::Swap(VPiecePath& path) noexcept { std::swap(d, path.d); }
...
//---------------------------------------------------------------------------------------------------------------------
void VPiecePath::Clear() { d->m_nodes.clear(); }

//---------------------------------------------------------------------------------------------------------------------
qint32 VPiecePath::CountNodes() const { return d->m_nodes.size(); }

//---------------------------------------------------------------------------------------------------------------------
VPieceNode& VPiecePath::operator[](int indx) { return d->m_nodes[indx]; }
...
//---------------------------------------------------------------------------------------------------------------------
PiecePathType VPiecePath::GetType() const { return d->m_type; }
...
//---------------------------------------------------------------------------------------------------------------------
Qt::PenStyle VPiecePath::GetPenType() const { return d->m_penType; }

//---------------------------------------------------------------------------------------------------------------------
void VPiecePath::SetPenType(const Qt::PenStyle& type) { d->m_penType = type; }

//---------------------------------------------------------------------------------------------------------------------
bool VPiecePath::IsCutPath() const { return d->m_cut; }

//---------------------------------------------------------------------------------------------------------------------
void VPiecePath::SetCutPath(bool cut) { d->m_cut = cut; }
    ...
    for (int i = 0; i < CountNodes(); ++i) {
        ...
        case Tool::NodePoint: {
            if (showExcluded || not at(i).isExcluded()) {
                points.append(*data->GeometricObject<VPointF>(at(i).GetId()));
            }
        } break;
        case Tool::NodeArc:
        case Tool::NodeElArc:
...
//---------------------------------------------------------------------------------------------------------------------
QVector<VSAPoint>
VPiecePath::SeamAllowancePoints(const VContainer* data, qreal width, bool reverse) const
{
...
//---------------------------------------------------------------------------------------------------------------------
VSAPoint VPiecePath::StartSegment(
    const VContainer* data, const QVector<VPieceNode>& nodes, int i, bool reverse)
{
    if (i < 0 || i > nodes.size() - 1) {
        return VSAPoint();
    }

    const auto& curve{ *data->GeometricObject<VAbstractCurve>(nodes.at(i).GetId()) };
...
//---------------------------------------------------------------------------------------------------------------------
VSAPoint VPiecePath::EndSegment(
    const VContainer* data, const QVector<VPieceNode>& nodes, int i, bool reverse)
{
    if (i < 0 || i > nodes.size() - 1) {
        return VSAPoint();
...
//---------------------------------------------------------------------------------------------------------------------
int VPiecePath::indexOfNode(quint32 id) const { return indexOfNode(d->m_nodes, id); }
...
//---------------------------------------------------------------------------------------------------------------------
/**
 * @brief OnEdge checks if two poins located on the edge. Edge is line between two points. If
 * between two points located arcs or splines ignore this.
...
VPiecePath VPiecePath::RemoveEdge(quint32 index) const
{
    ...
    for (quint32 i = 0; i < edges; ++i) {
        if (i == index) {
            ...
            for (int j = j1; j < j2;
                 ++j) {   // Add "segment" except last point. Inside can be curves too.
...
VSAPoint VPiecePath::PreparePointEkv(const VPieceNode& node, const VContainer* data)
{
    ...
    VSAPoint p{ data->GeometricObject<VPointF>(node.GetId())->toQPointF() };
...
//---------------------------------------------------------------------------------------------------------------------
QVector<VSAPoint> VPiecePath::CurveSeamAllowanceSegment(
    const VContainer* data,
    ...
    qreal width)
{
    ...
    if (w1 < 0 && w2 < 0) {   // no local widths
        for (int i = 0; i < points.size(); ++i) {
            ...
            } else if (i == points.size() - 1) {   // last point
