...
static void
compute_circle(point2d_t* pt0, point2d_t* pt1, point2d_t* pt2, real* cx, real* cy, real* radius)
{
...
/*
 * find the lower tangent between the two delaunay, going from left to right (returns the left half
 * edge)
 */
...
static void del_link(delaunay_t* result, delaunay_t* left, delaunay_t* right)
{
    ...
    while (del_classify_point(right->rightmost_he, right->rightmost_he->prev->pair->vertex)
           == ON_RIGHT)
    ...
    /*	if( d->face.radius < 0.0 )
        {
            ...
            {
                compute_circle(d->face.p[0], d->face.p[1], d->face.p[2], &(d->face.cx),
       &(d->face.cy), &(d->face.radius));
            }
        }
    */
}
