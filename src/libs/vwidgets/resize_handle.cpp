...
//------------------------------------------------------------------------------
void ResizeHandlesItem::setLockAspectRatio(bool lock) { m_lockAspectRatio = lock; }
...
void ResizeHandlesItem::parentIsLocked(bool lock)
{
    ...
    foreach (HandleItem* handleItem, m_handleItems) {
        ...
        handleItem->setRect(
            lock ? QRect(-lock_size / 2, -lock_size / 2, lock_size, lock_size)
                 : QRect(-HANDLE_SIZE / 2, -HANDLE_SIZE / 2, HANDLE_SIZE, HANDLE_SIZE));
    }
}


//------------------------------------------------------------------------------
void ResizeHandlesItem::setParentRotation(qreal rotation) { m_parentRotation = rotation; }
...
void ResizeHandlesItem::updateHandlePositions()
{
    foreach (HandleItem* item, m_handleItems) {
        ...
        case Position::TopLeft: item->setPos(m_parentRect.topLeft()); break;
        case Position::Top:
            item->setPos(m_parentRect.left() + m_parentRect.width() / 2 - 1, m_parentRect.top());
            break;
        case Position::TopRight: item->setPos(m_parentRect.topRight()); break;
        case Position::Right:
            item->setPos(m_parentRect.right(), m_parentRect.top() + m_parentRect.height() / 2 - 1);
            break;
        case Position::BottomRight: item->setPos(m_parentRect.bottomRight()); break;
        case Position::Bottom:
            item->setPos(m_parentRect.left() + m_parentRect.width() / 2 - 1, m_parentRect.bottom());
            break;
        case Position::BottomLeft: item->setPos(m_parentRect.bottomLeft()); break;
        case Position::Left:
            item->setPos(m_parentRect.left(), m_parentRect.top() + m_parentRect.height() / 2 - 1);
            break;
 ...
 * @return Position
 */
Position ResizeHandlesItem::HandleItem::position() const { return m_handlePosition; }
 ...
 * @return void.
 */
void ResizeHandlesItem::HandleItem::paint(
    QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
    Q_UNUSED(option);
    Q_UNUSED(widget);
    ...
    if (change == ItemPositionChange) {
        ...
        case Position::TopLeft:
            if (QGuiApplication::keyboardModifiers() & Qt::ControlModifier) {
                ...
                if (m_parent->m_lockAspectRatio
                    ...
                    xdiff = ((m_parent->m_parentRect.bottom() + ydiff - pos.y()) * m_scalingFactor
                             - m_parent->m_parentRect.width())
                          / 2;
                ...
                m_parent->m_lockAspectRatio
                || QGuiApplication::keyboardModifiers() & Qt::ShiftModifier) {
                pos.setX(
                    m_parent->m_parentRect.right()
                    - (m_parent->m_parentRect.bottom() - pos.y()) * m_scalingFactor);
            }
        ...
        case Position::Top:
            if (QGuiApplication::keyboardModifiers() & Qt::ControlModifier) {
                ...
                if (m_parent->m_lockAspectRatio
                    || QGuiApplication::keyboardModifiers() & Qt::ShiftModifier) {
                    qreal xdiff =
                        ((m_parent->m_parentRect.bottom() + ydiff - pos.y()) * m_scalingFactor
                         - m_parent->m_parentRect.width())
                        / 2;
                    m_parent->setRight(m_parent->m_parentRect.right() + xdiff);
        ...
        case Position::Right:
            if (QGuiApplication::keyboardModifiers() & Qt::ControlModifier) {
                ...
                if (m_parent->m_lockAspectRatio
                    || QGuiApplication::keyboardModifiers() & Qt::ShiftModifier) {
                    qreal ydiff =
                        ((pos.x() - m_parent->m_parentRect.left() + xdiff) / m_scalingFactor
                         - m_parent->m_parentRect.height())
                        / 2;
                    m_parent->setBottom(m_parent->m_parentRect.bottom() + ydiff);
                ...
                pos.setY(
                    ...
                    + (pos.x() - m_parent->m_parentRect.left()) / m_scalingFactor);
        ...
        case Position::BottomRight:
            if (QGuiApplication::keyboardModifiers() & Qt::ControlModifier) {
                ...
                if (m_parent->m_lockAspectRatio
                    || QGuiApplication::keyboardModifiers() & Qt::ShiftModifier) {
                    xdiff = ((pos.y() - m_parent->m_parentRect.top() + ydiff) * m_scalingFactor
                             - m_parent->m_parentRect.width())
                          / 2;
                    pos.setX(m_parent->m_parentRect.right() + xdiff);
                }
                ...
                pos.setX(
                    m_parent->m_parentRect.left()
                    + (pos.y() - m_parent->m_parentRect.top()) * m_scalingFactor);
            }
        ...
        case Position::Bottom:
            if (QGuiApplication::keyboardModifiers() & Qt::ControlModifier) {
                ...
                if (m_parent->m_lockAspectRatio
                    || QGuiApplication::keyboardModifiers() & Qt::ShiftModifier) {
                    qreal xdiff =
                        ((pos.y() - m_parent->m_parentRect.top() + ydiff) * m_scalingFactor
                         - m_parent->m_parentRect.width())
                        / 2;
                    m_parent->setRight(m_parent->m_parentRect.right() + xdiff);
                    m_parent->setLeft(m_parent->m_parentRect.left() - xdiff);
...
{
    QPointF point = pos();
    if (m_handlePosition == Position::TopLeft || m_handlePosition == Position::BottomRight
        || m_handlePosition == Position::TopRight || m_handlePosition == Position::BottomLeft) {
        point.setX(newPos.x());
        point.setY(newPos.y());
    }
    ...
    bool isMovingTop = m_handlePosition == Position::Top || m_handlePosition == Position::TopLeft
    ...
    bool isMovingLeft = m_handlePosition == Position::Left || m_handlePosition == Position::TopLeft
                     || m_handlePosition == Position::BottomLeft;
    ...
    if (isMovingTop) {
        ...
        if (m_parent->m_lockAspectRatio && m_scalingFactor < 1
            && height * m_scalingFactor < m_minDimension) {
            point.setY(m_parent->m_parentRect.bottom() - m_minDimension / m_scalingFactor);
        } else if (
            m_parent->m_lockAspectRatio && m_scalingFactor > 1
            && height * m_scalingFactor > m_maxDimension) {
    ...
    if (isMovingLeft) {
        ...
        if (m_parent->m_lockAspectRatio && m_scalingFactor > 1
            && width / m_scalingFactor < m_minDimension) {
            point.setX(m_parent->m_parentRect.right() - m_minDimension * m_scalingFactor);
        } else if (
            m_parent->m_lockAspectRatio && m_scalingFactor < 1
            && width / m_scalingFactor > m_maxDimension) {
...
void ResizeHandlesItem::HandleItem::hoverEnterEvent(QGraphicsSceneHoverEvent* event)
{
    ...
    if (!m_parent->m_parentIsLocked) {
        ...
        transform.rotate(
            m_parent->m_parentRotation + (static_cast<int>(m_handlePosition) - 1) * 45);
        ...
        if (m_parent->m_lockAspectRatio) {
            message =
                tr("Press <b>CTRL</b> to scale around the center - <b> Aspect ratio locked </b>");
        } else {
            message =
                tr("Press <b>CTRL</b> to scale around the center,"
