...
{
    // We need to check current scale factor because in Windows we have an error when we zoom in or
    // zoom out to much. See issue #532: Unexpected error occurs when zoom out image. factor > 1 for
    // zoomIn and factor < 1 for zoomOut.
    const qreal m11 = m_view->transform().m11();

    if ((factor > 1 && m11 <= VMainGraphicsView::MaxScale())
        || (factor < 1 && m11 >= VMainGraphicsView::MinScale())) {
        m_view->scale(factor, factor);
        if (factor < 1) {
            // Because QGraphicsView centers the picture when it's smaller than the view. And
            // QGraphicsView's scrolls boundaries don't allow to put any picture point at any
            // viewport position we will provide fictive scene size. Temporary and bigger than view,
            // scene size will help position an image under cursor.
            fictiveSceneRect(m_view->scene(), m_view);
        }
        m_view->centerOn(targetScenePos);
        QPointF deltaViewPos =
            targetViewPos
...
void GraphicsViewZoom::setModifiers(Qt::KeyboardModifiers modifiers) { m_modifiers = modifiers; }

//---------------------------------------------------------------------------------------------------------------------
// cppcheck-suppress unusedFunction
void GraphicsViewZoom::setZoomSpeedFactor(qreal value) { m_zoomSpeedFactor = value; }
...
void GraphicsViewZoom::verticalScrollingTime(qreal x)
{
    ...
    qreal scroll = static_cast<qreal>(qAbs(m_numScheduledVerticalScrollings))
                 * (10. + 10. / m_view->transform().m22())
...
void GraphicsViewZoom::horizontalScrollingTime(qreal x)
{
    ...
    qreal scroll = static_cast<qreal>(qAbs(m_numScheduledHorizontalScrollings))
                 * (10. + 10. / m_view->transform().m11())
                 / (static_cast<qreal>(m_duration) / static_cast<qreal>(m_updateInterval));
    ...
    m_view->horizontalScrollBar()->setValue(
        qRound(m_view->horizontalScrollBar()->value() + scroll));
}
...
void GraphicsViewZoom::animFinished()
{
    ...
    /*
     * In moust cases cursor position on view doesn't change, but for scene after scrolling position
     * will be different. We are goint to check changes and save new value. If don't do that we will
     * zoom using old value cursor position on scene. It is not what we expect. Almoust the same we
     * do in method GraphicsViewZoom::eventFilter.
     */
...
bool GraphicsViewZoom::eventFilter(QObject* object, QEvent* event)
{
    ...
    const qreal m_zoomSpeedFactor =
        (qreal(qApp->Settings()->getZoomSpeedFactor()) / 10000) + 1.0004;
...
void GraphicsViewZoom::panTriggered(QPanGesture* gesture)
{
    ...
    case Qt::GestureUpdated: m_view->viewport()->setCursor(Qt::SizeAllCursor); break;
    default: m_view->viewport()->setCursor(Qt::ArrowCursor);
    }
#endif
...
bool GraphicsViewZoom::startVerticalScrollings(QWheelEvent* wheel_event)
{
    ...
    if (m_numScheduledVerticalScrollings * numSteps
        < 0) {   // if user moved the wheel in another direction, we reset previously scheduled
                 // scalings
        m_numScheduledVerticalScrollings = numSteps;
    }
...
bool GraphicsViewZoom::startHorizontalScrollings(QWheelEvent* wheel_event)
{
    ...
    if (m_numScheduledHorizontalScrollings * numSteps
        < 0) {   // if user moved the wheel in another direction, we reset previously scheduled
                 // scalings
        m_numScheduledHorizontalScrollings = numSteps;
    }
    ...
    , zoom(new GraphicsViewZoom(this))
{
    ...
    connect(zoom, &GraphicsViewZoom::zoomed, this, [this]() {
...
{
    // We need to check current scale factor because in Windows we get an error when we zoom in or
    // out too much. See issue #532: Unexpected error occurs when zoom out image.
...
{
    // We need to check current scale factor because in Windows we have an error when we zoom in or
    // zoom out to much. See issue #532: Unexpected error occurs when zoom out image.
...
void VMainGraphicsView::mouseReleaseEvent(QMouseEvent* event)
{
    ...
    if (isPanDragActive) {
        ...
        if (rubberBandRect.width() > 5 && rubberBandRect.height() > 5) {
            this->fitInView(
                QRectF(
                    mapToScene(rubberBandRect.topLeft()), mapToScene(rubberBandRect.bottomRight())),
                Qt::KeepAspectRatio);
            updateView(this->transform());
        }
...
//---------------------------------------------------------------------------------------------------------------------
void VMainGraphicsView::setShowToolOptions(bool value) { showToolProperties = value; }

//---------------------------------------------------------------------------------------------------------------------
void VMainGraphicsView::allowRubberBand(bool value) { isallowRubberBand = value; }

//---------------------------------------------------------------------------------------------------------------------
/**
 * @brief NewSceneRect calculate scene rect what contains all items and doesn't less that size of
 * scene view.
 * @param sc scene.
 * @param view view.
 */
...
void VMainGraphicsView::resetScrollBars() { initScrollBars(); }

//---------------------------------------------------------------------------------------------------------------------
void VMainGraphicsView::resetScrollAnimations() { zoom->initScrollAnimations(); }
