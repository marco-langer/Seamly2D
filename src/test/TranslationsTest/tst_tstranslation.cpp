...
void TST_TSTranslation::CheckEnglishLocalization_data()
{
    ...
    for (qint32 i = 0, num = messages.size(); i < num; ++i) {
        ...
        if (message.isNull() == false) {
            ...
            const QString message = QString("File '%1'. Check modification source message '%2'.")
                                        .arg(fileName)
                                        .arg(source);
...
void TST_TSTranslation::CheckEmptyToolButton_data()
{
    ...
    for (qint32 i = 0, num = messages.size(); i < num; ++i) {
        ...
        if (message.isNull() == false) {
            ...
            const QString tag = QString("File '%1'. Check modification source message '%2'.")
                                    .arg(fileName)
                                    .arg(source);
    ...
    for (int j = 0; j < locales.size(); ++j) {
        ...
        for (qint32 i = 0, num = messages.size(); i < num; ++i) {
            ...
            if (message.isNull() == false) {
                ...
                const QString message = QString("File '%1'. Check place holder source message '%2'")
                                            .arg(filename)
                                            .arg(source);
                QTest::newRow(qUtf8Printable(message)) << source << translation;
            } else {
                const QString message =
                    QString("File '%2'. Message %1 is null.").arg(i).arg(filename);
                QFAIL(qUtf8Printable(message));
            }
        }
    }
    ...
    for (int i = 1; i <= 99; ++i) {
        ...
        if (sourceMark) {
            ...
            if (sourceMarkCount != i) {
                const QString message =
                    QString("In source string '%1' was missed place marker ").arg(source)
        ...
        if (translationMark) {
            ...
            if (translationMarkCount != i) {
                const QString message =
                    ...
                    + QLatin1String("'%") + QString().setNum(translationMarkCount)
                    + QLatin1String("'.");
                QFAIL(qUtf8Printable(message));
            }
        }
        ...
        if (sourceMark != translationMark) {
            const QString message = QString(
                                        "Compare to source string in the translation string '%1' "
                                        "was missed place marker ")
                                        .arg(translation)
                                  + QLatin1String("'%") + QString().setNum(sourceMarkCount)
                                  + QLatin1String("'.");
            QFAIL(qUtf8Printable(message));
        }
    }
...
void TST_TSTranslation::TestPunctuation()
{
    ...
    static const QStringList punctuation =
        QStringList() << QLatin1String(".") << QLatin1String(":") << QLatin1String(" ")
    ...
    if (punctuation.contains(cSource)) {
        if (not translation.endsWith(cSource)) {
            ...
            if (locale == QLatin1String("el_GR") && cSource == QLatin1Char('?')
                && cTranslation == QLatin1Char(';')) {
                testFail = false;
            }
        }
    } else {
    ...
    if (testFail) {
        const QString message =
            QString(
                "Translation string does not end with the same punctuation character '%1' or "
                "vice versa. ")
                .arg(cPunctuation)
...
void TST_TSTranslation::TestHTMLTags()
{
    ...
    static const QStringList tags = QStringList() << QLatin1String("p") << QLatin1String("html")
                                                  << QLatin1String("body");
    static const QString pattern("{1}.*>");
    for (int i = 0; i < tags.size(); ++i) {
        const QRegularExpression openRegex(
            QLatin1String("<") + tags.at(i) + pattern,
            QRegularExpression::DotMatchesEverythingOption);
        if (source.contains(openRegex)) {
            const int countOpenTag = source.count(openRegex);
            const QRegularExpression closeRegex(
                QLatin1String("</") + tags.at(i) + pattern,
                QRegularExpression::DotMatchesEverythingOption);
