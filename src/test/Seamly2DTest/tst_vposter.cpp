...
void TST_VPoster::BigPoster()
{
    ...
    // We need to set full page because otherwise QPrinter->pageRect returns different values in
    // Windows and Linux
...
QRect TST_VPoster::PageRect(const QPrinter& printer) const
{
    ...
    QRect pageRect(
        qFloor(ToPixel(rect.x())),
        qFloor(ToPixel(rect.y())),
        qFloor(ToPixel(rect.width())),
