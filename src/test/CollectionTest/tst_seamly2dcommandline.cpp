...
void TST_Seamly2DCommandLine::OpenPatterns_data() const
{
    ...
    QTest::newRow("Send wrong path to a file") << "wrongPath.sm2d" << V_EX_NOINPUT;

    QTest::newRow("Measurement independent empty file") << "empty.sm2d" << V_EX_OK;

    QTest::newRow("File with invalid object type") << "wrong_obj_type.sm2d" << V_EX_NOINPUT;

    QTest::newRow("Empty text Seamly 2D file") << "txt.sm2d" << V_EX_NOINPUT;
}
...
void TST_Seamly2DCommandLine::OpenPatterns()
{
    ...
    const int exit =
        Run(exitCode,
            Seamly2DPath(),
            QStringList() << "--test" << tmp + QDir::separator() + file,
            error);
...
void TST_Seamly2DCommandLine::ExportMode_data() const
{
    ...
    QTest::newRow("Issue #372") << "issue_372.sm2d" << QString("-p;;0;;-d;;%1;;-b;;output").arg(tmp)
                                << V_EX_OK;
    ...
    QTest::newRow("A file with limited gradation. Multisize measurements. Correct data.")
        << "glimited_vst.sm2d"
        << QString("-p;;0;;-d;;%1;;--gsize;;40;;--gheight;;134;;-b;;output").arg(tmp) << V_EX_OK;
...
void TST_Seamly2DCommandLine::ExportMode()
{
    ...
    const QStringList arg = QStringList()
                         << tmp + QDir::separator() + file << arguments.split(";;");
...
void TST_Seamly2DCommandLine::TestMode_data() const
{
    ...
    QTest::newRow("Issue #256. Correct path.") << "issue_256.sm2d" << QString("--test") << V_EX_OK;

    QTest::newRow("Issue #256. Wrong path.")
        << "issue_256_wrong_path.smis" << QString("--test") << V_EX_NOINPUT;

    QTest::newRow("Issue #256. Correct individual measurements.")
        << "issue_256.sm2d"
        << QString("--test;;-m;;%1")
               .arg(tmp + QDir::separator() + QLatin1String("issue_256_correct.smis"))
        << V_EX_OK;

    QTest::newRow("Issue #256. Wrong individual measurements.")
        << "issue_256.sm2d"
        << QString("--test;;-m;;%1")
               .arg(tmp + QDir::separator() + QLatin1String("issue_256_wrong.smis"))
        << V_EX_NOINPUT;

    QTest::newRow("Issue #256. Correct multisize measurements.")
        << "issue_256.sm2d"
        << QString("--test;;-m;;%1")
               .arg(tmp + QDir::separator() + QLatin1String("issue_256_correct.smms"))
        << V_EX_OK;

    QTest::newRow("Issue #256. Wrong multisize measurements.")
        << "issue_256.sm2d"
        << QString("--test;;-m;;%1")
               .arg(tmp + QDir::separator() + QLatin1String("issue_256_wrong.smms"))
        << V_EX_NOINPUT;

    QTest::newRow("Wrong formula.") << "wrong_formula.sm2d" << QString("--test") << V_EX_DATAERR;
}
...
void TST_Seamly2DCommandLine::TestOpenCollection_data() const
{
    ...
    const QString testGOST =
        QString("--test;;-m;;%1").arg(tmp + QDir::separator() + QLatin1String("gost_man_ru.smms"));
    ...
    QTest::newRow("IMK_Zhaketa") << "imk_zhaketa_poluprilegayuschego_silueta.sm2d" << keyTest
                                 << V_EX_OK;
    QTest::newRow("moulage_05_armhole_neckline")
        << "moulage_05_armhole_neckline.sm2d" << keyTest << V_EX_OK;
    QTest::newRow("07_armhole_adjustment_010")
        << "07_armhole_adjustment_010.sm2d" << keyTest << V_EX_OK;
}
